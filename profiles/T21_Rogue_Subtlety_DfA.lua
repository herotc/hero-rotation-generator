...
local function APL()
  ...
  local function Cds()
    ...
    if S.ArcaneTorrent:IsCastableP() and HR.CDsON() and (bool(stealthed.rogue) and Player:EnergyDeficit() > 70) then
    ...
    if S.ShadowBlades:IsCastableP() and ((HL.CombatTime() > 10 and Player:ComboPointsDeficit() >= 2 + stealthed.all - num(I.MantleoftheMasterAssassin:IsEquipped())) or (HL.CombatTime() < 10 and (not S.MarkedForDeath:IsAvailable() or Player:ComboPointsDeficit() >= 3 or Target:DebuffP(S.NightbladeDebuff)))) then
    ...
    if S.GoremawsBite:IsCastableP() and (not bool(stealthed.all) and S.ShadowDance:ChargesFractional() <= VarShdFractional and ((Player:ComboPointsDeficit() >= 4 - num((HL.CombatTime() < 10)) * 2 and Player:EnergyDeficit() > 50 + num(S.Vigor:IsAvailable()) * 25 - num((HL.CombatTime() >= 10)) * 15) or (Player:ComboPointsDeficit() >= 1 and Target:TimeToDie() < 8))) then
      if HR.Cast(S.GoremawsBite) then return ""; end
    end
  ...
  local function Finish()
    ...
    if S.DeathFromAbove:IsCastableP() and (not S.DarkShadow:IsAvailable() or (not Player:BuffP(S.ShadowDanceBuff) or Cache.EnemiesCount[15] >= 4) and (Player:BuffP(S.SymbolsofDeathBuff) or S.SymbolsofDeath:CooldownRemainsP() >= 10 + num(HL.Tier20_4Pc) * 5) and Player:BuffRemainsP(S.TheFirstoftheDeadBuff) < 1 and (Player:BuffP(S.FinalityEviscerateBuff) or Cache.EnemiesCount[10] < 4)) then
  ...
  local function StealthCds()
    ...
    if S.Vanish:IsCastableP() and (not bool(VarDshDfa) and mantle_duration == 0 and S.ShadowDance:ChargesFractional() < VarShdFractional + num((I.MantleoftheMasterAssassin:IsEquipped() and HL.CombatTime() < 30)) * 0.3 and (not I.MantleoftheMasterAssassin:IsEquipped() or Player:BuffP(S.SymbolsofDeathBuff))) then
      if HR.Cast(S.Vanish) then return ""; end
    end
    ...
    if S.ShadowDance:IsCastableP() and (not bool(VarDshDfa) and Player:ComboPointsDeficit() >= 2 + num(S.Subterfuge:IsAvailable()) * 2 and (Player:BuffRemainsP(S.SymbolsofDeathBuff) >= 1.2 or S.SymbolsofDeath:CooldownRemainsP() >= 12 + num((S.DarkShadow:IsAvailable() and HL.Tier20_4Pc)) * 3 - num((not S.DarkShadow:IsAvailable() and HL.Tier20_4Pc)) * 4 or mantle_duration > 0) and (Cache.EnemiesCount[10] >= 4 or not Player:BuffP(S.TheFirstoftheDeadBuff))) then
      if HR.Cast(S.ShadowDance) then return ""; end
    end
  end
  ...
  if (S.DarkShadow:IsAvailable() and Player:ComboPointsDeficit() >= 2 + num(Player:BuffP(S.ShadowBladesBuff)) and (Target:DebuffRemainsP(S.NightbladeDebuff) > 4 + num(S.Subterfuge:IsAvailable()) or S.ShadowDance:ChargesFractional() >= 1.9 and (not I.DenialoftheHalfgiants:IsEquipped() or HL.CombatTime() > 10))) then
  ...
  if (Player:ComboPoints() >= 5 + 3 * num((Player:BuffP(S.TheFirstoftheDeadBuff) and S.Anticipation:IsAvailable())) + num((S.DeeperStratagem:IsAvailable() and not Player:BuffP(S.ShadowBladesBuff) and (mantle_duration == 0 or HL.Tier20_4Pc) and (not Player:BuffP(S.TheFirstoftheDeadBuff) or bool(VarDshDfa)))) or (Player:ComboPoints() >= 4 and Player:ComboPointsDeficit() <= 2 and Cache.EnemiesCount[10] >= 3 and Cache.EnemiesCount[10] <= 4) or (Target:TimeToDie() <= 1 and Player:ComboPoints() >= 3)) then
    local ShouldReturn = Finish(); if ShouldReturn then return ShouldReturn; end
  end
