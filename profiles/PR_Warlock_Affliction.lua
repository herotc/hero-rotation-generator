...
local function APL()
  ...
  Precombat = function()
    ...
    if S.Haunt:IsCastableP() and Player:DebuffDownP(S.HauntDebuff) then
      if HR.Cast(S.Haunt) then return "haunt 18"; end
    end
  ...
  Fillers = function()
    ...
    if S.Agony:IsCastableP() and (S.Deathbolt:IsAvailable() and S.SummonDarkglare:CooldownRemainsP() >= 30 + Player:GCD() and S.Deathbolt:CooldownRemainsP() <= Player:GCD() and not Player:PrevGCDP(1, S.SummonDarkglare) and not Player:PrevGCDP(1, S.Agony) and S.WritheInAgony:IsAvailable() and S.SuddenOnset:AzeriteEnabled() and Target:DebuffRemainsP(S.AgonyDebuff) < S.AgonyDebuff:BaseDuration() * 0.5) then
      if HR.Cast(S.Agony) then return "agony 28"; end
    end
    ...
    if S.Deathbolt:IsCastableP() and (S.SummonDarkglare:CooldownRemainsP() >= 30 + Player:GCD() or S.SummonDarkglare:CooldownRemainsP() > 140) then
      if HR.Cast(S.Deathbolt) then return "deathbolt 56"; end
    end
    ...
    if S.ShadowBolt:IsCastableP() and (Player:BuffP(S.MovementBuff) and bool(Player:BuffRemainsP(S.NightfallBuff))) then
      if HR.Cast(S.ShadowBolt) then return "shadow_bolt 62"; end
    end
    ...
    if S.SiphonLife:IsCastableP() and (Player:BuffP(S.MovementBuff) and not (Player:PrevGCDP(1, S.SiphonLife) and Player:PrevGCDP(2, S.SiphonLife) and Player:PrevGCDP(3, S.SiphonLife))) then
      if HR.Cast(S.SiphonLife) then return "siphon_life 82"; end
    end
    ...
    if S.Corruption:IsCastableP() and (Player:BuffP(S.MovementBuff) and not Player:PrevGCDP(1, S.Corruption) and not S.AbsoluteCorruption:IsAvailable()) then
      if HR.Cast(S.Corruption) then return "corruption 92"; end
    end
    ...
    if S.DrainLife:IsCastableP() and ((Player:BuffStackP(S.InevitableDemiseBuff) >= 90 and (S.Deathbolt:CooldownRemainsP() > S.DrainLife:ExecuteTime() or not S.Deathbolt:IsAvailable()) and (S.PhantomSingularity:CooldownRemainsP() > S.DrainLife:ExecuteTime() or not S.PhantomSingularity:IsAvailable()) and (S.DarkSoul:CooldownRemainsP() > S.DrainLife:ExecuteTime() or not S.DarkSoulMisery:IsAvailable()) and (S.VileTaint:CooldownRemainsP() > S.DrainLife:ExecuteTime() or not S.VileTaint:IsAvailable()) and S.SummonDarkglare:CooldownRemainsP() > S.DrainLife:ExecuteTime() + 10 or Player:BuffStackP(S.InevitableDemiseBuff) > 30 and Target:TimeToDie() <= 10)) then
      if HR.Cast(S.DrainLife) then return "drain_life 100"; end
    end
    ...
    if S.DrainSoul:IsCastableP() then
      if HR.CastCycle(S.DrainSoul, 40, function(TargetUnit) return TargetUnit:TimeToDie() <= Player:GCD() end) then return "drain_soul 148" end
    end
    ...
    if S.DrainSoul:IsCastableP() then
      if HR.CastTargetIf(S.DrainSoul, 40, "min", function(TargetUnit) return TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff) end, function(TargetUnit) return S.ShadowEmbrace:IsAvailable() and Cache.EnemiesCount[40] == 2 and not bool(TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff)) end) then return "drain_soul 167" end
    end
    ...
    if S.DrainSoul:IsCastableP() then
      if HR.CastTargetIf(S.DrainSoul, 40, "min", function(TargetUnit) return TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff) end, function(TargetUnit) return S.ShadowEmbrace:IsAvailable() and Cache.EnemiesCount[40] == 2 end) then return "drain_soul 184" end
    end
    -- drain_soul,interrupt_global=1,chain=1,interrupt=1
    if S.DrainSoul:IsCastableP() then
      if HR.Cast(S.DrainSoul) then return "drain_soul 185"; end
    end
    ...
    if S.ShadowBolt:IsCastableP() then
      if HR.CastCycle(S.ShadowBolt, 40, function(TargetUnit) return S.ShadowEmbrace:IsAvailable() and S.AbsoluteCorruption:IsAvailable() and Cache.EnemiesCount[40] == 2 and not bool(TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff)) and not S.ShadowBolt:InFlight() end) then return "shadow_bolt 207" end
    end
  ...
  if Everyone.TargetIsValid() then
    ...
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions and ((S.DarkSoulMisery:IsAvailable() and S.SummonDarkglare:CooldownUpP() and S.DarkSoul:CooldownUpP()) or S.SummonDarkglare:CooldownUpP() or TargetUnit:TimeToDie() < 30) then
    ...
    if S.SummonDarkglare:IsCastableP() and HR.CDsON() and (TargetUnit:DebuffP(S.AgonyDebuff) and TargetUnit:DebuffP(S.CorruptionDebuff) and (ActiveUAs() == 5 or Player:SoulShardsP() == 0) and (not S.PhantomSingularity:IsAvailable() or bool(S.PhantomSingularity:CooldownRemainsP()))) then
    ...
    if S.Agony:IsCastableP() then
      if HR.CastTargetIf(S.Agony, 40, "min", function(TargetUnit) return TargetUnit:DebuffRemainsP(S.AgonyDebuff) end, function(TargetUnit) return TargetUnit:DebuffRemainsP(S.AgonyDebuff) <= Player:GCD() + S.ShadowBolt:ExecuteTime() end) then return "agony 307" end
    end
    ...
    if S.PhantomSingularity:IsCastableP() and (HL.CombatTime() > 40 and (S.SummonDarkglare:CooldownRemainsP() >= 45 or S.SummonDarkglare:CooldownRemainsP() < 8)) then
      if HR.Cast(S.PhantomSingularity, Settings.Affliction.GCDasOffGCD.PhantomSingularity) then return "phantom_singularity 362"; end
    end
    -- vile_taint,if=time>20
    if S.VileTaint:IsCastableP() and (HL.CombatTime() > 20) then
      if HR.Cast(S.VileTaint) then return "vile_taint 368"; end
    end
    ...
    if S.SeedofCorruption:IsCastableP() and (TargetUnit:DebuffRemainsP(S.CorruptionDebuff) <= S.SeedofCorruption:CastTime() + time_to_shard + 4.2 * (1 - num(S.CreepingDeath:IsAvailable()) * 0.15) and Cache.EnemiesCount[5] >= 3 + num(S.WritheInAgony:IsAvailable()) and not bool(TargetUnit:DebuffRemainsP(S.SeedofCorruptionDebuff)) and not S.SeedofCorruption:InFlight()) then
      if HR.Cast(S.SeedofCorruption) then return "seed_of_corruption 370"; end
    end
    -- agony,cycle_targets=1,max_cycle_targets=6,if=talent.creeping_death.enabled&target.time_to_die>10&refreshable
    if S.Agony:IsCastableP() then
      if HR.CastCycle(S.Agony, 40, function(TargetUnit) return S.CreepingDeath:IsAvailable() and TargetUnit:TimeToDie() > 10 and TargetUnit:DebuffRefreshableCP(S.AgonyDebuff) end) then return "agony 400" end
    end
    -- agony,cycle_targets=1,max_cycle_targets=8,if=(!talent.creeping_death.enabled)&target.time_to_die>10&refreshable
    if S.Agony:IsCastableP() then
      if HR.CastCycle(S.Agony, 40, function(TargetUnit) return (not S.CreepingDeath:IsAvailable()) and TargetUnit:TimeToDie() > 10 and TargetUnit:DebuffRefreshableCP(S.AgonyDebuff) end) then return "agony 413" end
    end
    ...
    if S.Corruption:IsCastableP() then
      if HR.CastCycle(S.Corruption, 40, function(TargetUnit) return Cache.EnemiesCount[40] < 3 + num(S.WritheInAgony:IsAvailable()) and TargetUnit:DebuffRefreshableCP(S.CorruptionDebuff) and TargetUnit:TimeToDie() > 10 end) then return "corruption 574" end
    end
    -- phantom_singularity,if=time<=40
    if S.PhantomSingularity:IsCastableP() and (HL.CombatTime() <= 40) then
      if HR.Cast(S.PhantomSingularity, Settings.Affliction.GCDasOffGCD.PhantomSingularity) then return "phantom_singularity 575"; end
    end
    -- vile_taint
    if S.VileTaint:IsCastableP() then
      if HR.Cast(S.VileTaint) then return "vile_taint 577"; end
    end
    -- dark_soul
    if S.DarkSoul:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.DarkSoul, Settings.Affliction.GCDasOffGCD.DarkSoul) then return "dark_soul 579"; end
    end
    -- berserking
    if S.Berserking:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return "berserking 581"; end
    end
    -- unstable_affliction,if=soul_shard>=5
    if S.UnstableAffliction:IsReadyP() and (Player:SoulShardsP() >= 5) then
      if HR.Cast(S.UnstableAffliction) then return "unstable_affliction 583"; end
    end
    ...
    if S.UnstableAffliction:IsReadyP() and (S.SummonDarkglare:CooldownRemainsP() <= Player:SoulShardsP() * S.UnstableAffliction:ExecuteTime()) then
      if HR.Cast(S.UnstableAffliction) then return "unstable_affliction 585"; end
    end
    ...
    if ((S.SummonDarkglare:CooldownRemainsP() < time_to_shard * (5 - Player:SoulShardsP()) or S.SummonDarkglare:CooldownUpP()) and TargetUnit:TimeToDie() > S.SummonDarkglare:CooldownRemainsP()) then
    ...
    if S.SeedofCorruption:IsCastableP() and (bool(VarSpammableSeed)) then
      if HR.Cast(S.SeedofCorruption) then return "seed_of_corruption 607"; end
    end
    ...
    if S.UnstableAffliction:IsReadyP() and (not Player:PrevGCDP(1, S.SummonDarkglare) and not bool(VarSpammableSeed) and (S.Deathbolt:IsAvailable() and S.Deathbolt:CooldownRemainsP() <= S.UnstableAffliction:ExecuteTime() and not S.CascadingCalamity:AzeriteEnabled() or Player:SoulShardsP() >= 2 and TargetUnit:TimeToDie() > 4 + S.UnstableAffliction:ExecuteTime() and Cache.EnemiesCount[40] == 1 or TargetUnit:TimeToDie() <= 8 + S.UnstableAffliction:ExecuteTime() * Player:SoulShardsP())) then
      if HR.Cast(S.UnstableAffliction) then return "unstable_affliction 611"; end
    end
    ...
    if S.UnstableAffliction:IsReadyP() and (not bool(VarSpammableSeed) and contagion <= S.UnstableAffliction:CastTime() + VarPadding) then
      if HR.Cast(S.UnstableAffliction) then return "unstable_affliction 649"; end
    end
    ...
    if S.UnstableAffliction:IsReadyP() then
      if HR.CastCycle(S.UnstableAffliction, 40, function(TargetUnit) return not bool(VarSpammableSeed) and (not S.Deathbolt:IsAvailable() or S.Deathbolt:CooldownRemainsP() > time_to_shard or Player:SoulShardsP() > 1) and contagion <= S.UnstableAffliction:CastTime() + VarPadding and (not S.CascadingCalamity:AzeriteEnabled() or Player:BuffRemainsP(S.CascadingCalamityBuff) > time_to_shard) end) then return "unstable_affliction 683" end
    end
