...
local function Apl()
  local function Cooldown()
    ...
    if S.Nemesis:IsCastable() and (bool(raid_event.adds.exists) and Player:DebuffDown(S.NemesisDebuff) and (active_enemies > desired_targets or raid_event.adds.in > 60)) then
    ...
    if S.Nemesis:IsCastable() and (not bool(raid_event.adds.exists) and (Player:Buff(S.ChaosBladesBuff) or Player:Buff(S.MetamorphosisBuff) or cooldown.metamorphosis.adjusted_remains < 20 or Target:TimeToDie() <= 60)) then
      if AR.Cast(S.Nemesis) then return ""; end
    end
  ...
  local function Demonic()
    ...
    if S.BladeDance:IsCastable() and (bool(BladeDance) and S.EyeBeam:CooldownRemainsP() > 5 and not S.Metamorphosis:IsReady()) then
    ...
    if S.Annihilation:IsCastable() and IsInMeleeRange() and ((not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff) or Player:FuryDeficit() < 30 + num(Player:Buff(S.PreparedBuff)) * 8 or Player:BuffRemainsP(S.MetamorphosisBuff) < 5) and not bool(PoolingForBladeDance)) then
    ...
    if S.ChaosStrike:IsCastable() and IsInMeleeRange() and ((not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff) or Player:FuryDeficit() < 30 + num(Player:Buff(S.PreparedBuff)) * 8) and not bool(PoolingForChaosStrike) and not bool(PoolingForMeta) and not bool(PoolingForBladeDance)) then
  ...
  local function Normal()
    ...
    if S.Felblade:IsCastable() and (Player:FuryDeficit() >= 30 + num(Player:Buff(S.PreparedBuff)) * 8) then
      if AR.Cast(S.Felblade) then return ""; end
    end
    ...
    if S.Annihilation:IsCastable() and IsInMeleeRange() and ((S.DemonBlades:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff) or Player:FuryDeficit() < 30 + num(Player:Buff(S.PreparedBuff)) * 8 or Player:BuffRemainsP(S.MetamorphosisBuff) < 5) and not bool(PoolingForBladeDance)) then
      if AR.Cast(S.Annihilation) then return ""; end
    end
    -- throw_glaive,if=talent.bloodlet.enabled&(!talent.master_of_the_glaive.enabled|!talent.momentum.enabled|buff.momentum.up)&raid_event.adds.in>recharge_time+cooldown
    if S.ThrowGlaive:IsCastable() and (S.Bloodlet:IsAvailable() and (not S.MasterofTheGlaive:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff)) and raid_event.adds.in > S.ThrowGlaive:RechargeP() + S.ThrowGlaive:CooldownRemainsP()) then
    ...
    if S.ChaosStrike:IsCastable() and IsInMeleeRange() and ((S.DemonBlades:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff) or Player:FuryDeficit() < 30 + num(Player:Buff(S.PreparedBuff)) * 8) and not bool(PoolingForChaosStrike) and not bool(PoolingForMeta) and not bool(PoolingForBladeDance)) then
      if AR.Cast(S.ChaosStrike) then return ""; end
    end
