...
local function Apl()
  local function Cooldown()
    ...
    if S.Metamorphosis:IsCastable(S.Metamorphosis) and (not (S.Demonic:IsAvailable() or bool(PoolingForMeta) or bool(WaitingForNemesis) or bool(WaitingForChaosBlades)) or Target:TimeToDie() < 25) then
      if AR.Cast(S.Metamorphosis) then return ""; end
    end
    -- metamorphosis,if=talent.demonic.enabled&buff.metamorphosis.up&fury<40
    if S.Metamorphosis:IsCastable(S.Metamorphosis) and (S.Demonic:IsAvailable() and Player:Buff(S.MetamorphosisBuff) and Player:Fury() < 40) then
      if AR.Cast(S.Metamorphosis) then return ""; end
    end
    -- nemesis,target_if=min:target.time_to_die,if=raid_event.adds.exists&debuff.nemesis.down&(active_enemies>desired_targets|raid_event.adds.in>60)
    if S.Nemesis:IsCastable(S.Nemesis) and (bool(raid_event.adds.exists) and Player:DebuffDown(S.NemesisDebuff) and (active_enemies > desired_targets or raid_event.adds.in > 60)) then
    ...
    if S.Nemesis:IsCastable(S.Nemesis) and (not bool(raid_event.adds.exists) and (Player:Buff(S.ChaosBladesBuff) or Player:Buff(S.MetamorphosisBuff) or cooldown.metamorphosis.adjusted_remains < 20 or Target:TimeToDie() <= 60)) then
      if AR.Cast(S.Nemesis) then return ""; end
    end
    -- chaos_blades,if=buff.metamorphosis.up|cooldown.metamorphosis.adjusted_remains>60|target.time_to_die<=duration
    if S.ChaosBlades:IsCastable(S.ChaosBlades) and (Player:Buff(S.MetamorphosisBuff) or cooldown.metamorphosis.adjusted_remains > 60 or Target:TimeToDie() <= duration) then
  ...
  local function Demonic()
    ...
    if S.PickUpFragment:IsCastable(S.PickUpFragment) and (Player:FuryDeficit() >= 35 and (S.EyeBeam:CooldownRemainsP() > 5 or Player:Buff(S.MetamorphosisBuff))) then
    ...
    if S.VengefulRetreat:IsCastable(S.VengefulRetreat) and ((S.Prepared:IsAvailable() or S.Momentum:IsAvailable()) and Player:BuffDown(S.PreparedBuff) and Player:BuffDown(S.MomentumBuff)) then
    ...
    if S.FelRush:IsCastable(S.FelRush) and ((S.Momentum:IsAvailable() or S.FelMastery:IsAvailable()) and (not S.Momentum:IsAvailable() or (S.FelRush:ChargesP() == 2 or S.VengefulRetreat:CooldownRemainsP() > 4) and Player:BuffDown(S.MomentumBuff)) and (S.FelRush:ChargesP() == 2 or (raid_event.movement.in > 10 and raid_event.adds.in > 10))) then
    ...
    if S.ThrowGlaive:IsCastable(S.ThrowGlaive) and (S.Bloodlet:IsAvailable() and (not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff)) and S.ThrowGlaive:ChargesP() == 2) then
      if AR.Cast(S.ThrowGlaive) then return ""; end
    end
    -- death_sweep,if=variable.blade_dance
    if S.DeathSweep:IsCastable(S.DeathSweep) and (bool(BladeDance)) then
      if AR.Cast(S.DeathSweep) then return ""; end
    end
    -- fel_eruption
    if S.FelEruption:IsCastable(S.FelEruption) and (true) then
    ...
    if S.FuryofTheIllidari:IsCastable(S.FuryofTheIllidari) and ((active_enemies > desired_targets) or (raid_event.adds.in > 55 and (not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff)))) then
    ...
    if S.BladeDance:IsCastable(S.BladeDance) and (bool(BladeDance) and S.EyeBeam:CooldownRemainsP() > 5 and not S.Metamorphosis:IsReady()) then
    ...
    if S.ThrowGlaive:IsCastable(S.ThrowGlaive) and (S.Bloodlet:IsAvailable() and spell_targets >= 2 and (not S.MasterofTheGlaive:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff)) and (spell_targets >= 3 or raid_event.adds.in > S.ThrowGlaive:RechargeP() + S.ThrowGlaive:CooldownRemainsP())) then
      if AR.Cast(S.ThrowGlaive) then return ""; end
    end
    ...
    if S.EyeBeam:IsCastable(S.EyeBeam) and (spell_targets.eye_beam_tick > desired_targets or not bool(buff.metamorphosis.extended_by_demonic) or (AC.Tier21_4Pc and Player:BuffRemainsP(S.MetamorphosisBuff) > 8)) then
    ...
    if S.Annihilation:IsCastable(S.Annihilation) and IsInMeleeRange() and ((not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff) or Player:FuryDeficit() < 30 + num(Player:Buff(S.PreparedBuff)) * 8 or Player:BuffRemainsP(S.MetamorphosisBuff) < 5) and not bool(PoolingForBladeDance)) then
    ...
    if S.ThrowGlaive:IsCastable(S.ThrowGlaive) and (S.Bloodlet:IsAvailable() and (not S.MasterofTheGlaive:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff)) and raid_event.adds.in > S.ThrowGlaive:RechargeP() + S.ThrowGlaive:CooldownRemainsP()) then
      if AR.Cast(S.ThrowGlaive) then return ""; end
    end
    ...
    if S.ChaosStrike:IsCastable(S.ChaosStrike) and IsInMeleeRange() and ((not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff) or Player:FuryDeficit() < 30 + num(Player:Buff(S.PreparedBuff)) * 8) and not bool(PoolingForChaosStrike) and not bool(PoolingForMeta) and not bool(PoolingForBladeDance)) then
    ...
    if S.FelRush:IsCastable(S.FelRush) and (not S.Momentum:IsAvailable() and (Player:BuffDown(S.MetamorphosisBuff) or S.DemonBlades:IsAvailable()) and (S.FelRush:ChargesP() == 2 or (raid_event.movement.in > 10 and raid_event.adds.in > 10))) then
      if AR.Cast(S.FelRush) then return ""; end
    end
    -- demons_bite
    ...
    if S.ThrowGlaive:IsCastable(S.ThrowGlaive) and (Player:Buff(S.OutofRangeBuff) or not S.Bloodlet:IsAvailable()) then
      if AR.Cast(S.ThrowGlaive) then return ""; end
    end
    -- fel_rush,if=movement.distance>15|(buff.out_of_range.up&!talent.momentum.enabled)
    if S.FelRush:IsCastable(S.FelRush) and (movement.distance > 15 or (Player:Buff(S.OutofRangeBuff) and not S.Momentum:IsAvailable())) then
      if AR.Cast(S.FelRush) then return ""; end
    end
    -- vengeful_retreat,if=movement.distance>15
    if S.VengefulRetreat:IsCastable(S.VengefulRetreat) and (movement.distance > 15) then
      if AR.Cast(S.VengefulRetreat) then return ""; end
    end
  end
  local function Normal()
    -- pick_up_fragment,if=talent.demonic_appetite.enabled&fury.deficit>=35
    if S.PickUpFragment:IsCastable(S.PickUpFragment) and (S.DemonicAppetite:IsAvailable() and Player:FuryDeficit() >= 35) then
      if AR.Cast(S.PickUpFragment) then return ""; end
    end
    -- vengeful_retreat,if=(talent.prepared.enabled|talent.momentum.enabled)&buff.prepared.down&buff.momentum.down
    if S.VengefulRetreat:IsCastable(S.VengefulRetreat) and ((S.Prepared:IsAvailable() or S.Momentum:IsAvailable()) and Player:BuffDown(S.PreparedBuff) and Player:BuffDown(S.MomentumBuff)) then
    ...
    if S.FelBarrage:IsCastable(S.FelBarrage) and ((Player:Buff(S.MomentumBuff) or not S.Momentum:IsAvailable()) and (active_enemies > desired_targets or raid_event.adds.in > 30)) then
    ...
    if S.Felblade:IsCastable(S.Felblade) and (Player:Fury() < 15 and (S.DeathSweep:CooldownRemainsP() < 2 * Player:GCD() or S.BladeDance:CooldownRemainsP() < 2 * Player:GCD())) then
      if AR.Cast(S.Felblade) then return ""; end
    end
    -- death_sweep,if=variable.blade_dance
    if S.DeathSweep:IsCastable(S.DeathSweep) and (bool(BladeDance)) then
    ...
    if S.FelRush:IsCastable(S.FelRush) and (S.FelRush:ChargesP() == 2 and not S.Momentum:IsAvailable() and not S.FelMastery:IsAvailable() and not Player:Buff(S.MetamorphosisBuff)) then
      if AR.Cast(S.FelRush) then return ""; end
    end
    -- fel_eruption
    if S.FelEruption:IsCastable(S.FelEruption) and (true) then
    ...
    if S.Felblade:IsCastable(S.Felblade) and (Player:FuryDeficit() >= 30 + num(Player:Buff(S.PreparedBuff)) * 8) then
      if AR.Cast(S.Felblade) then return ""; end
    end
    ...
    if S.Annihilation:IsCastable(S.Annihilation) and IsInMeleeRange() and ((S.DemonBlades:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff) or Player:FuryDeficit() < 30 + num(Player:Buff(S.PreparedBuff)) * 8 or Player:BuffRemainsP(S.MetamorphosisBuff) < 5) and not bool(PoolingForBladeDance)) then
      if AR.Cast(S.Annihilation) then return ""; end
    end
    -- throw_glaive,if=talent.bloodlet.enabled&(!talent.master_of_the_glaive.enabled|!talent.momentum.enabled|buff.momentum.up)&raid_event.adds.in>recharge_time+cooldown
    if S.ThrowGlaive:IsCastable(S.ThrowGlaive) and (S.Bloodlet:IsAvailable() and (not S.MasterofTheGlaive:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff)) and raid_event.adds.in > S.ThrowGlaive:RechargeP() + S.ThrowGlaive:CooldownRemainsP()) then
    ...
    if S.ThrowGlaive:IsCastable(S.ThrowGlaive) and (not S.Bloodlet:IsAvailable() and Player:BuffDown(S.MetamorphosisBuff) and spell_targets >= 3) then
      if AR.Cast(S.ThrowGlaive) then return ""; end
    end
    ...
    if S.ChaosStrike:IsCastable(S.ChaosStrike) and IsInMeleeRange() and ((S.DemonBlades:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.MomentumBuff) or Player:FuryDeficit() < 30 + num(Player:Buff(S.PreparedBuff)) * 8) and not bool(PoolingForChaosStrike) and not bool(PoolingForMeta) and not bool(PoolingForBladeDance)) then
      if AR.Cast(S.ChaosStrike) then return ""; end
    end
    ...
    if S.FelRush:IsCastable(S.FelRush) and (not S.Momentum:IsAvailable() and raid_event.movement.in > S.FelRush:ChargesP() * 10 and (S.DemonBlades:IsAvailable() or Player:BuffDown(S.MetamorphosisBuff))) then
      if AR.Cast(S.FelRush) then return ""; end
    end
    -- demons_bite
    if S.DemonsBite:IsCastable(S.DemonsBite) and IsInMeleeRange() and (true) then
    ...
    if S.ThrowGlaive:IsCastable(S.ThrowGlaive) and (Player:Buff(S.OutofRangeBuff)) then
      if AR.Cast(S.ThrowGlaive) then return ""; end
    end
    -- felblade,if=movement.distance>15|buff.out_of_range.up
    if S.Felblade:IsCastable(S.Felblade) and (movement.distance > 15 or Player:Buff(S.OutofRangeBuff)) then
      if AR.Cast(S.Felblade) then return ""; end
    end
    -- fel_rush,if=movement.distance>15|(buff.out_of_range.up&!talent.momentum.enabled)
    if S.FelRush:IsCastable(S.FelRush) and (movement.distance > 15 or (Player:Buff(S.OutofRangeBuff) and not S.Momentum:IsAvailable())) then
  ...
  -- auto_attack
  if S.AutoAttack:IsCastable(S.AutoAttack) and (true) then
  ...
  if (true) then
    WaitingForChaosBlades = num(not (not S.ChaosBlades:IsAvailable() or S.ChaosBlades:IsReady() or S.ChaosBlades:CooldownRemainsP() > Target:TimeToDie() or S.ChaosBlades:CooldownRemainsP() > 60))
  end
  ...
  -- consume_magic
  if S.ConsumeMagic:IsCastable(S.ConsumeMagic) and (true) then
