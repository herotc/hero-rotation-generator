...
local function APL()
  ...
  Precombat = function()
    ...
    if S.SeedofCorruption:IsCastableP() and (Cache.EnemiesCount[40] >= 3) then
    ...
    if S.ShadowBolt:IsCastableP() and (not S.Haunt:IsAvailable() and Cache.EnemiesCount[40] < 3) then
  ...
  if not Player:AffectingCombat() and not Player:IsCasting() then
    ...
    if S.SeedofCorruption:IsCastableP() and (Target:DebuffTicksRemainP(S.CorruptionDebuff) <= 2 and Cache.EnemiesCount[40] >= 3) then
      if HR.Cast(S.SeedofCorruption) then return ""; end
    end
    -- phantom_singularity
    ...
    if (S.SowtheSeeds:IsAvailable() and Cache.EnemiesCount[40] >= 3) then
