...
local function APL()
  ...
  Fnb = function()
    ...
    if S.Immolate:IsCastableP() and (not bool(Target:DebuffRemainsP(S.HavocDebuff)) and Target:DebuffRefreshableCP(S.ImmolateDebuff) and Cache.EnemiesCount[40] <= 8) then
      if HR.Cast(S.Immolate) then return ""; end
    end
    -- rain_of_fire
    if S.RainofFire:IsCastableP() then
    ...
    if S.SoulFire:IsCastableP() and (not bool(Target:DebuffRemainsP(S.HavocDebuff)) and Cache.EnemiesCount[40] == 3) then
      if HR.Cast(S.SoulFire) then return ""; end
    end
    ...
    if S.Conflagrate:IsCastableP() and (not bool(Target:DebuffRemainsP(S.HavocDebuff)) and (S.Flashover:IsAvailable() and Player:BuffStackP(S.BackdraftBuff) <= 2 or Cache.EnemiesCount[40] <= 7 or S.RoaringBlaze:IsAvailable() and Cache.EnemiesCount[40] <= 9)) then
      if HR.Cast(S.Conflagrate) then return ""; end
    end
    -- incinerate,cycle_targets=1,if=!debuff.havoc.remains
    if S.Incinerate:IsCastableP() and (not bool(Target:DebuffRemainsP(S.HavocDebuff))) then
