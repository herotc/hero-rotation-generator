...
local function APL()
  ...
  Burn = function()
    ...
    if S.ArcaneBlast:IsReadyP() and (Player:BuffP(S.PresenceofMindBuff) and HL.Tier20_2Pc and S.Overpowered:IsAvailable() and Player:BuffP(S.ArcanePowerBuff)) then
      if HR.Cast(S.ArcaneBlast) then return ""; end
    end
  ...
  Conserve = function()
    ...
    if S.ArcaneBlast:IsReadyP() and ((Player:BuffP(S.RuleofThreesBuff) or bool(Player:BuffStackP(S.RhoninsAssaultingArmwrapsBuff))) and Player:ArcaneChargesP() >= 3) then
      if HR.Cast(S.ArcaneBlast) then return ""; end
    end
    ...
    if S.ArcaneBlast:IsReadyP() and (I.MysticKiltoftheRuneMaster:IsEquipped() and Player:ArcaneChargesP() == 0) then
      if HR.Cast(S.ArcaneBlast) then return ""; end
    end
