...
local function Apl()
  ...
  local function Haunt()
    ...
    if S.SoulHarvest:IsCastableP() and (Player:BuffRemainsP(S.SoulHarvestBuff) <= 8 and Player:BuffStackP(S.ActiveUasBuff) >= 1 and (raid_event.adds.in > 20 or Cache.EnemiesCount[40] > 1 or not bool(raid_event.adds.exists))) then
  ...
  local function Writhe()
    ...
    if S.SoulHarvest:IsCastableP() and (sim.target == target and Player:BuffRemainsP(S.SoulHarvestBuff) <= 8 and (raid_event.adds.in > 20 or Cache.EnemiesCount[40] > 1 or not bool(raid_event.adds.exists)) and (Player:BuffStackP(S.ActiveUasBuff) >= 2 or Cache.EnemiesCount[40] > 3) and (not S.DeathsEmbrace:IsAvailable() or Target:TimeToDie() > 120 or Target:TimeToDie() < 30)) then
      if AR.Cast(S.SoulHarvest) then return ""; end
    end
    -- potion,if=target.time_to_die<=70
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions and (Target:TimeToDie() <= 70) then
