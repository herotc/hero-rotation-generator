...
Spell.Warlock.Affliction = {
  ReapSouls                             = Spell(),
  DeadwindHarvesterBuff                 = Spell(),
  TormentedSoulsBuff                    = Spell(),
  HauntDebuff                           = Spell(),
  SowtheSeeds                           = Spell(),
  Agony                                 = Spell(),
  DrainSoul                             = Spell(),
  ServicePet                            = Spell(),
  CorruptionDebuff                      = Spell(),
  AgonyDebuff                           = Spell(),
  ...
  PhantomSingularity                    = Spell(),
  MaleficGrasp                          = Spell(),
  UnstableAffliction1                   = Spell(),
  SeedofCorruption                      = Spell(),
  Contagion                             = Spell(),
  ...
  DeathsEmbrace                         = Spell(),
  ConcordanceoftheLegionfallBuff        = Spell(),
  WritheInAgony                         = Spell()
};
...
local function Apl()
  local function Haunt()
    ...
    if S.ReapSouls:IsCastableP() and (Cache.EnemiesCount[5] > 1 and not bool(Player:BuffRemainsP(S.DeadwindHarvesterBuff)) and AC.CombatTime() > 5 and soul_shard > 0 and ((S.SowtheSeeds:IsAvailable() and Cache.EnemiesCount[5] >= 3) or Cache.EnemiesCount[5] >= 5)) then
      if AR.Cast(S.ReapSouls) then return ""; end
    end
    ...
    if S.SoulHarvest:IsCastableP() and (Player:BuffRemainsP(S.SoulHarvestBuff) <= 8 and Player:BuffStackP(S.ActiveUasBuff) >= 1 and (raid_event.adds.in > 20 or Cache.EnemiesCount[5] > 1 or not bool(raid_event.adds.exists))) then
    ...
    if S.Corruption:IsCastableP() and (Player:BuffRemainsP(S.Corruption) <= S.Corruption:TickTime() + Player:GCD() and (Cache.EnemiesCount[5] < 3 and S.SowtheSeeds:IsAvailable() or Cache.EnemiesCount[5] < 5)) then
    ...
    if S.Agony:IsCastableP() and (Player:BuffRemainsP(S.Agony) <= S.Agony:BaseDuration() * 0.3 and Target:TimeToDie() >= Player:BuffRemainsP(S.Agony)) then
      if AR.Cast(S.Agony) then return ""; end
    end
    ...
    if S.SiphonLife:IsCastableP() and (Player:BuffRemainsP(S.SiphonLife) <= S.SiphonLife:BaseDuration() * 0.3 and Target:TimeToDie() >= Player:BuffRemainsP(S.SiphonLife)) then
      if AR.Cast(S.SiphonLife) then return ""; end
    end
    ...
    if S.SeedofCorruption:IsCastableP() and (S.SowtheSeeds:IsAvailable() and Cache.EnemiesCount[5] >= 3 or Cache.EnemiesCount[5] >= 5 or Cache.EnemiesCount[5] >= 3 and Target:DebuffRemainsP(S.CorruptionDebuff) <= S.SeedofCorruption:CastTime() + S.SeedofCorruption:TravelTime()) then
      if AR.Cast(S.SeedofCorruption) then return ""; end
    end
    -- corruption,if=remains<=duration*0.3&target.time_to_die>=remains
    if S.Corruption:IsCastableP() and (Player:BuffRemainsP(S.Corruption) <= S.Corruption:BaseDuration() * 0.3 and Target:TimeToDie() >= Player:BuffRemainsP(S.Corruption)) then
      if AR.Cast(S.Corruption) then return ""; end
    end
    ...
    if S.UnstableAffliction:IsCastableP() and ((not S.SowtheSeeds:IsAvailable() or Cache.EnemiesCount[5] < 3) and Cache.EnemiesCount[5] < 5 and ((soul_shard >= 4 and not S.Contagion:IsAvailable()) or soul_shard >= 5 or Target:TimeToDie() < 30)) then
    ...
    if S.UnstableAffliction:IsCastableP() and (Cache.EnemiesCount[5] > 1 and (not S.SowtheSeeds:IsAvailable() or Cache.EnemiesCount[5] < 3) and (I.Item132381:IsEquipped() or I.Item132457:IsEquipped()) and S.Haunt:CooldownRemainsP() < 15 and Target:DebuffRemainsP(S.UnstableAffliction1Debuff) < S.UnstableAffliction:CastTime() and Target:DebuffRemainsP(S.UnstableAffliction2Debuff) < S.UnstableAffliction:CastTime() and Target:DebuffRemainsP(S.UnstableAffliction3Debuff) < S.UnstableAffliction:CastTime() and Target:DebuffRemainsP(S.UnstableAffliction4Debuff) < S.UnstableAffliction:CastTime() and Target:DebuffRemainsP(S.UnstableAffliction5Debuff) < S.UnstableAffliction:CastTime()) then
      if AR.Cast(S.UnstableAffliction) then return ""; end
    end
    ...
    if S.UnstableAffliction:IsCastableP() and ((not S.SowtheSeeds:IsAvailable() or Cache.EnemiesCount[5] < 3) and Cache.EnemiesCount[5] < 5 and S.Contagion:IsAvailable() and soul_shard >= 4 and Target:DebuffRemainsP(S.UnstableAffliction1Debuff) < S.UnstableAffliction:CastTime() and Target:DebuffRemainsP(S.UnstableAffliction2Debuff) < S.UnstableAffliction:CastTime() and Target:DebuffRemainsP(S.UnstableAffliction3Debuff) < S.UnstableAffliction:CastTime() and Target:DebuffRemainsP(S.UnstableAffliction4Debuff) < S.UnstableAffliction:CastTime() and Target:DebuffRemainsP(S.UnstableAffliction5Debuff) < S.UnstableAffliction:CastTime()) then
      if AR.Cast(S.UnstableAffliction) then return ""; end
    end
    ...
    if S.UnstableAffliction:IsCastableP() and ((not S.SowtheSeeds:IsAvailable() or Cache.EnemiesCount[5] < 3) and Cache.EnemiesCount[5] < 5 and Target:DebuffRemainsP(S.HauntDebuff) >= S.UnstableAffliction1:TickTime() * 2) then
      if AR.Cast(S.UnstableAffliction) then return ""; end
    end
  ...
  local function Mg()
    ...
    if S.ReapSouls:IsCastableP() and (not bool(Player:BuffRemainsP(S.DeadwindHarvesterBuff)) and AC.CombatTime() > 5 and ((Player:BuffStackP(S.TormentedSoulsBuff) >= 4 + Cache.EnemiesCount[5] or Player:BuffStackP(S.TormentedSoulsBuff) >= 9) or Target:TimeToDie() <= Player:BuffStackP(S.TormentedSoulsBuff) * (5 + 1.5 * num(I.Item144364:IsEquipped())) + (Player:BuffRemainsP(S.DeadwindHarvesterBuff) * (5 + 1.5 * num(I.Item144364:IsEquipped())) / 12 * (5 + 1.5 * num(I.Item144364:IsEquipped()))))) then
      if AR.Cast(S.ReapSouls) then return ""; end
    end
    ...
    if S.SeedofCorruption:IsCastableP() and (S.SowtheSeeds:IsAvailable() and Cache.EnemiesCount[5] >= 3 and soul_shard == 5) then
      if AR.Cast(S.SeedofCorruption) then return ""; end
    end
    ...
    if S.Corruption:IsCastableP() and ((not S.SowtheSeeds:IsAvailable() or Cache.EnemiesCount[5] < 3) and Cache.EnemiesCount[5] < 5 and Player:BuffRemainsP(S.Corruption) <= (S.Corruption:TickTime() + Player:GCD()) and Target:TimeToDie() > S.Corruption:TickTime() * 3) then
      if AR.Cast(S.Corruption) then return ""; end
    end
    -- phantom_singularity
    if S.PhantomSingularity:IsCastableP() and (true) then
    ...
    if S.Agony:IsCastableP() and (Player:BuffRemainsP(S.Agony) <= (S.Agony:BaseDuration() * 0.3) and Target:TimeToDie() >= Player:BuffRemainsP(S.Agony) and (Player:BuffStackP(S.ActiveUasBuff) == 0 or Player:PrevGCDP(1, S.Agony))) then
      if AR.Cast(S.Agony) then return ""; end
    end
    ...
    if S.SiphonLife:IsCastableP() and (Player:BuffRemainsP(S.SiphonLife) <= (S.SiphonLife:BaseDuration() * 0.3) and Target:TimeToDie() >= Player:BuffRemainsP(S.SiphonLife) and (Player:BuffStackP(S.ActiveUasBuff) == 0 or Player:PrevGCDP(1, S.SiphonLife))) then
      if AR.Cast(S.SiphonLife) then return ""; end
    end
    ...
    if S.Corruption:IsCastableP() and ((not S.SowtheSeeds:IsAvailable() or Cache.EnemiesCount[5] < 3) and Cache.EnemiesCount[5] < 5 and Player:BuffRemainsP(S.Corruption) <= (S.Corruption:BaseDuration() * 0.3) and Target:TimeToDie() >= Player:BuffRemainsP(S.Corruption) and (Player:BuffStackP(S.ActiveUasBuff) == 0 or Player:PrevGCDP(1, S.Corruption))) then
      if AR.Cast(S.Corruption) then return ""; end
    end
    -- life_tap,if=talent.empowered_life_tap.enabled&buff.empowered_life_tap.remains<duration*0.3|talent.malefic_grasp.enabled&target.time_to_die>15&mana.pct<10
    if S.LifeTap:IsCastableP() and (S.EmpoweredLifeTap:IsAvailable() and Player:BuffRemainsP(S.EmpoweredLifeTapBuff) < S.LifeTap:BaseDuration() * 0.3 or S.MaleficGrasp:IsAvailable() and Target:TimeToDie() > 15 and Player:ManaPercentage() < 10) then
    ...
    if S.SeedofCorruption:IsCastableP() and ((S.SowtheSeeds:IsAvailable() and Cache.EnemiesCount[5] >= 3) or (Cache.EnemiesCount[5] >= 5 and Target:DebuffRemainsP(S.CorruptionDebuff) <= S.SeedofCorruption:CastTime() + S.SeedofCorruption:TravelTime())) then
      if AR.Cast(S.SeedofCorruption) then return ""; end
    end
    ...
    if S.UnstableAffliction:IsCastableP() and (target == sim.target and Cache.EnemiesCount[5] > 1 and soul_shard >= 4) then
      if AR.Cast(S.UnstableAffliction) then return ""; end
    end
  ...
  local function Writhe()
    ...
    if S.SoulHarvest:IsCastableP() and (sim.target == target and Player:BuffRemainsP(S.SoulHarvestBuff) <= 8 and (raid_event.adds.in > 20 or Cache.EnemiesCount[5] > 1 or not bool(raid_event.adds.exists)) and (Player:BuffStackP(S.ActiveUasBuff) >= 2 or Cache.EnemiesCount[5] > 3) and (not S.DeathsEmbrace:IsAvailable() or Target:TimeToDie() > 120 or Target:TimeToDie() < 30)) then
      if AR.Cast(S.SoulHarvest) then return ""; end
    end
    -- potion,if=target.time_to_die<=70
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions and (Target:TimeToDie() <= 70) then
    ...
    if S.Corruption:IsCastableP() and (Player:BuffRemainsP(S.Corruption) <= S.Corruption:TickTime() + Player:GCD() and ((Cache.EnemiesCount[5] < 3 and S.SowtheSeeds:IsAvailable()) or Cache.EnemiesCount[5] < 5) and Target:TimeToDie() > S.Corruption:TickTime() * 2) then
      if AR.Cast(S.Corruption) then return ""; end
    end
    ...
    if S.SeedofCorruption:IsCastableP() and ((S.SowtheSeeds:IsAvailable() and Cache.EnemiesCount[5] >= 3) or (Cache.EnemiesCount[5] > 3 and bool(dot.corruption.refreshable))) then
      if AR.Cast(S.SeedofCorruption) then return ""; end
    end
