...
local function APL()
  ...
  local function Haunt()
    ...
    if S.ReapSouls:IsCastableP() and (Cache.EnemiesCount[40] > 1 and not bool(Player:BuffRemainsP(S.DeadwindHarvesterBuff)) and AC.CombatTime() > 5 and FutureShard() > 0 and ((S.SowtheSeeds:IsAvailable() and Cache.EnemiesCount[40] >= 3) or Cache.EnemiesCount[40] >= 5)) then
      if AR.Cast(S.ReapSouls) then return ""; end
    end
  ...
  local function Writhe()
    ...
    if S.UnstableAffliction:IsCastableP() and (S.AbsoluteCorruption:IsAvailable() and AC.Tier21_4Pc and Target:DebuffRemainsP(S.TormentedAgonyDebuff) <= S.UnstableAffliction:CastTime()) then
      if AR.Cast(S.UnstableAffliction) then return ""; end
    end
    ...
    if S.UnstableAffliction:IsCastableP() and (Player:BuffRemainsP(S.DeadwindHarvesterBuff) > S.UnstableAffliction:TickTime() * 2 and (not AC.Tier21_4Pc or S.Contagion:IsAvailable() or FutureShard() > 1) and (not S.Contagion:IsAvailable() or FutureShard() > 1 or bool(Player:BuffRemainsP(S.SoulHarvestBuff))) and (num(Target:DebuffP(S.UnstableAffliction1Debuff)) + num(Target:DebuffP(S.UnstableAffliction2Debuff)) + num(Target:DebuffP(S.UnstableAffliction3Debuff)) + num(Target:DebuffP(S.UnstableAffliction4Debuff)) + num(Target:DebuffP(S.UnstableAffliction5Debuff)) < 5)) then
      if AR.Cast(S.UnstableAffliction) then return ""; end
    end
