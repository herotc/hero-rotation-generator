...
local function APL()
  ...
  Precombat = function()
    ...
    if S.ApplyPoison:IsCastableP() then
      if HR.Cast(S.ApplyPoison) then return "apply_poison 6269"; end
    end
    -- stealth
    if S.Stealth:IsCastableP() then
      if HR.Cast(S.Stealth) then return "stealth 6271"; end
    end
    -- potion
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 6273"; end
    end
    -- marked_for_death,precombat_seconds=5,if=raid_event.adds.in>40
    if S.MarkedForDeath:IsCastableP() and (10000000000 > 40) then
      if HR.Cast(S.MarkedForDeath) then return "marked_for_death 6275"; end
    end
  end
  Cds = function()
    ...
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions and (Player:HasHeroism() or Target:TimeToDie() <= 60 or Target:DebuffP(S.VendettaDebuff) and S.Vanish:CooldownRemainsP() < 5) then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 6277"; end
    end
    ...
    if I.GalecallersBoon:IsReady() and (S.Vendetta:CooldownRemainsP() <= 1 and (not S.Subterfuge:IsAvailable() or Target:PMultiplier(S.Garrote) > 1) or S.Vendetta:CooldownRemainsP() > 45) then
      if HR.CastSuggested(I.GalecallersBoon) then return "galecallers_boon 6283"; end
    end
    ...
    if S.Berserking:IsCastableP() and HR.CDsON() and (Target:DebuffP(S.VendettaDebuff)) then
      if HR.Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return "berserking 6299"; end
    end
    -- fireblood,if=debuff.vendetta.up
    if S.Fireblood:IsCastableP() and HR.CDsON() and (Target:DebuffP(S.VendettaDebuff)) then
      if HR.Cast(S.Fireblood, Settings.Commons.OffGCDasOffGCD.Racials) then return "fireblood 6303"; end
    end
    -- ancestral_call,if=debuff.vendetta.up
    if S.AncestralCall:IsCastableP() and HR.CDsON() and (Target:DebuffP(S.VendettaDebuff)) then
      if HR.Cast(S.AncestralCall, Settings.Commons.OffGCDasOffGCD.Racials) then return "ancestral_call 6307"; end
    end
    ...
    if S.MarkedForDeath:IsCastableP() and (bool(min:target.time_to_die)) and ((Cache.EnemiesCount[15] > 1) and (Target:TimeToDie() < Player:ComboPointsDeficit() * 1.5 or Player:ComboPointsDeficit() >= cp_max_spend)) then
      if HR.Cast(S.MarkedForDeath) then return "marked_for_death 6311"; end
    end
    ...
    if S.MarkedForDeath:IsCastableP() and (10000000000 > 30 - raid_event.adds.duration and Player:ComboPointsDeficit() >= cp_max_spend) then
      if HR.Cast(S.MarkedForDeath) then return "marked_for_death 6315"; end
    end
    ...
    if S.Vendetta:IsCastableP() and (not bool(stealthed.rogue) and Target:DebuffP(S.RuptureDebuff) and (not S.Subterfuge:IsAvailable() or not S.ShroudedSuffocation:AzeriteEnabled() or Target:PMultiplier(S.Garrote) > 1)) then
      if HR.Cast(S.Vendetta) then return "vendetta 6317"; end
    end
    ...
    if S.Vanish:IsCastableP() and (S.Subterfuge:IsAvailable() and not Target:DebuffP(S.GarroteDebuff) and bool(VarSingleTarget)) then
      if HR.Cast(S.Vanish) then return "vanish 6329"; end
    end
    ...
    if S.Vanish:IsCastableP() and (S.Exsanguinate:IsAvailable() and (S.Nightstalker:IsAvailable() or S.Subterfuge:IsAvailable() and bool(VarSingleTarget)) and Player:ComboPoints() >= cp_max_spend and S.Exsanguinate:CooldownRemainsP() < 1 and (not S.Subterfuge:IsAvailable() or not S.ShroudedSuffocation:AzeriteEnabled() or Target:PMultiplier(S.Garrote) <= 1)) then
      if HR.Cast(S.Vanish) then return "vanish 6337"; end
    end
    ...
    if S.Vanish:IsCastableP() and (S.Nightstalker:IsAvailable() and not S.Exsanguinate:IsAvailable() and Player:ComboPoints() >= cp_max_spend and Target:DebuffP(S.VendettaDebuff)) then
      if HR.Cast(S.Vanish) then return "vanish 6357"; end
    end
    ...
    if S.Vanish:IsCastableP() and (S.Subterfuge:IsAvailable() and (not S.Exsanguinate:IsAvailable() or not bool(VarSingleTarget)) and not bool(stealthed.rogue) and S.Garrote:CooldownUpP() and Target:DebuffRefreshableCP(S.GarroteDebuff) and (Cache.EnemiesCount[10] <= 3 and Player:ComboPointsDeficit() >= 1 + Cache.EnemiesCount[10] or Cache.EnemiesCount[10] >= 4 and Player:ComboPointsDeficit() >= 4)) then
      if HR.Cast(S.Vanish) then return "vanish 6365"; end
    end
    ...
    if S.Vanish:IsCastableP() and (S.MasterAssassin:IsAvailable() and not bool(stealthed.all) and master_assassin_remains <= 0 and not Target:DebuffRefreshableCP(S.RuptureDebuff)) then
      if HR.Cast(S.Vanish) then return "vanish 6377"; end
    end
    ...
    if S.Exsanguinate:IsCastableP() and (Target:DebuffRemainsP(S.RuptureDebuff) > 4 + 4 * cp_max_spend and not Target:DebuffRefreshableCP(S.GarroteDebuff)) then
      if HR.Cast(S.Exsanguinate) then return "exsanguinate 6383"; end
    end
    -- toxic_blade,if=dot.rupture.ticking
    if S.ToxicBlade:IsCastableP() and (Target:DebuffP(S.RuptureDebuff)) then
      if HR.Cast(S.ToxicBlade) then return "toxic_blade 6389"; end
    end
  end
  Direct = function()
    ...
    if S.Envenom:IsCastableP() and (Player:ComboPoints() >= 4 + num(S.DeeperStratagem:IsAvailable()) and (Target:DebuffP(S.VendettaDebuff) or Target:DebuffP(S.ToxicBladeDebuff) or Player:EnergyDeficitPredicted() <= 25 + VarEnergyRegenCombined or not bool(VarSingleTarget)) and (not S.Exsanguinate:IsAvailable() or S.Exsanguinate:CooldownRemainsP() > 2)) then
      if HR.Cast(S.Envenom) then return "envenom 6393"; end
    end
    ...
    if S.PoisonedKnife:IsCastableP() and (bool(VarUseFiller) and Player:BuffStackP(S.SharpenedBladesBuff) >= 29) then
      if HR.Cast(S.PoisonedKnife) then return "poisoned_knife 6415"; end
    end
    ...
    if S.FanofKnives:IsCastableP() and (bool(VarUseFiller) and (Player:BuffStackP(S.HiddenBladesBuff) >= 19 or Cache.EnemiesCount[10] >= 4 + num((S.DoubleDose:AzeriteRank() > 2)) + stealthed.rogue)) then
      if HR.Cast(S.FanofKnives) then return "fan_of_knives 6421"; end
    end
    ...
    if S.FanofKnives:IsCastableP() and (not Target:DebuffP(S.DeadlyPoisonDotDebuff)) and (bool(VarUseFiller) and Cache.EnemiesCount[10] >= 3) then
      if HR.Cast(S.FanofKnives) then return "fan_of_knives 6429"; end
    end
    ...
    if S.Blindside:IsCastableP() and (bool(VarUseFiller) and (Player:BuffP(S.BlindsideBuff) or not S.VenomRush:IsAvailable())) then
      if HR.Cast(S.Blindside) then return "blindside 6435"; end
    end
    ...
    if S.Mutilate:IsCastableP() and (not Target:DebuffP(S.DeadlyPoisonDotDebuff)) and (bool(VarUseFiller) and Cache.EnemiesCount[10] == 2) then
      if HR.Cast(S.Mutilate) then return "mutilate 6443"; end
    end
    -- mutilate,if=variable.use_filler
    if S.Mutilate:IsCastableP() and (bool(VarUseFiller)) then
      if HR.Cast(S.Mutilate) then return "mutilate 6449"; end
    end
  end
  Dot = function()
    ...
    if S.Rupture:IsCastableP() and (S.Exsanguinate:IsAvailable() and ((Player:ComboPoints() >= cp_max_spend and S.Exsanguinate:CooldownRemainsP() < 1) or (not Target:DebuffP(S.RuptureDebuff) and (HL.CombatTime() > 10 or Player:ComboPoints() >= 2)))) then
      if HR.Cast(S.Rupture) then return "rupture 6453"; end
    end
    ...
    if S.Garrote:IsCastableP() and ((not S.Subterfuge:IsAvailable() or not (S.Vanish:CooldownUpP() and S.Vendetta:CooldownRemainsP() <= 4)) and Player:ComboPointsDeficit() >= 1 and Target:DebuffRefreshableCP(S.GarroteDebuff) and (pmultiplier <= 1 or Target:DebuffRemainsP(S.GarroteDebuff) <= S.GarroteDebuff:TickTime() and Cache.EnemiesCount[10] >= 3 + num(S.ShroudedSuffocation:AzeriteEnabled())) and (not bool(exsanguinated) or Target:DebuffRemainsP(S.GarroteDebuff) <= S.GarroteDebuff:TickTime() * 2 and Cache.EnemiesCount[10] >= 3 + num(S.ShroudedSuffocation:AzeriteEnabled())) and (Target:TimeToDie() - Target:DebuffRemainsP(S.GarroteDebuff) > 4 and Cache.EnemiesCount[10] <= 1 or Target:TimeToDie() - Target:DebuffRemainsP(S.GarroteDebuff) > 12)) then
      if S.Garrote:IsUsablePPool() then
        if HR.Cast(S.Garrote) then return "garrote 6466"; end
      else
        if HR.Cast(S.PoolResource) then return "pool_resource 6467"; end
      end
    end
    ...
    if S.CrimsonTempest:IsCastableP() and (Cache.EnemiesCount[15] >= 2 and Player:BuffRemainsP(S.CrimsonTempestBuff) < 2 + num((Cache.EnemiesCount[15] >= 5)) and Player:ComboPoints() >= 4) then
      if HR.Cast(S.CrimsonTempest) then return "crimson_tempest 6521"; end
    end
    ...
    if S.Rupture:IsCastableP() and (Player:ComboPoints() >= 4 and Target:DebuffRefreshableCP(S.RuptureDebuff) and (pmultiplier <= 1 or Target:DebuffRemainsP(S.RuptureDebuff) <= S.RuptureDebuff:TickTime() and Cache.EnemiesCount[10] >= 3 + num(S.ShroudedSuffocation:AzeriteEnabled())) and (not bool(exsanguinated) or Target:DebuffRemainsP(S.RuptureDebuff) <= S.RuptureDebuff:TickTime() * 2 and Cache.EnemiesCount[10] >= 3 + num(S.ShroudedSuffocation:AzeriteEnabled())) and Target:TimeToDie() - Target:DebuffRemainsP(S.RuptureDebuff) > 4) then
      if HR.Cast(S.Rupture) then return "rupture 6545"; end
    end
  end
  Stealthed = function()
    ...
    if S.Garrote:IsCastableP() and (S.Subterfuge:IsAvailable() and Target:DebuffRefreshableCP(S.GarroteDebuff) and Target:TimeToDie() - Target:DebuffRemainsP(S.GarroteDebuff) > 2) then
      if HR.Cast(S.Garrote) then return "garrote 6613"; end
    end
    ...
    if S.Garrote:IsCastableP() and (S.Subterfuge:IsAvailable() and Target:DebuffRemainsP(S.GarroteDebuff) <= 10 and pmultiplier <= 1 and Target:TimeToDie() - Target:DebuffRemainsP(S.GarroteDebuff) > 2) then
      if HR.Cast(S.Garrote) then return "garrote 6629"; end
    end
    ...
    if S.Rupture:IsCastableP() and (S.Subterfuge:IsAvailable() and S.ShroudedSuffocation:AzeriteEnabled() and not Target:DebuffP(S.RuptureDebuff)) then
      if HR.Cast(S.Rupture) then return "rupture 6645"; end
    end
    ...
    if S.Garrote:IsCastableP() and (S.Subterfuge:IsAvailable() and S.ShroudedSuffocation:AzeriteEnabled() and Target:TimeToDie() > Target:DebuffRemainsP(S.GarroteDebuff)) then
      if HR.Cast(S.Garrote) then return "garrote 6653"; end
    end
    -- pool_resource,for_next=1
    ...
    if S.Garrote:IsCastableP() and (S.Subterfuge:IsAvailable() and S.Exsanguinate:IsAvailable() and S.Exsanguinate:CooldownRemainsP() < 1 and Player:PrevGCDP(1, S.Rupture) and Target:DebuffRemainsP(S.RuptureDebuff) > 5 + 4 * cp_max_spend) then
      if S.Garrote:IsUsablePPool() then
        if HR.Cast(S.Garrote) then return "garrote 6666"; end
      else
        if HR.Cast(S.PoolResource) then return "pool_resource 6667"; end
      end
    end
  end
  -- call precombat
  ...
  if Everyone.TargetIsValid() then
    -- stealth
    if S.Stealth:IsCastableP() then
      if HR.Cast(S.Stealth) then return "stealth 6680"; end
    end
    ...
    if S.ArcaneTorrent:IsCastableP() and HR.CDsON() and (Player:EnergyDeficitPredicted() >= 15 + VarEnergyRegenCombined) then
      if HR.Cast(S.ArcaneTorrent, Settings.Commons.OffGCDasOffGCD.Racials) then return "arcane_torrent 6694"; end
    end
    -- arcane_pulse
    if S.ArcanePulse:IsCastableP() then
      if HR.Cast(S.ArcanePulse) then return "arcane_pulse 6698"; end
    end
    -- lights_judgment
    if S.LightsJudgment:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.LightsJudgment) then return "lights_judgment 6700"; end
    end
  end
end
