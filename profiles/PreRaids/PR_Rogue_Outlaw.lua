...
local function APL()
  ...
  Cds = function()
    ...
    if S.MarkedForDeath:IsCastableP() then
      if HR.CastTargetIf(S.MarkedForDeath, 35, "min", function(TargetUnit) return Target:TimeToDie() end, function(TargetUnit) return (Cache.EnemiesCount[35] > 1) and (TargetUnit:TimeToDie() < Player:ComboPointsDeficit() or not bool(stealthed.rogue) and Player:ComboPointsDeficit() >= cp_max_spend - 1) end) then return "marked_for_death 54" end
    end
    ...
    if S.BladeFlurry:IsCastableP() and (Cache.EnemiesCount[8] >= 2 and not Player:BuffP(S.BladeFlurryBuff) and (not (Cache.EnemiesCount[8] > 1) or 0 > 8 or 10000000000 > (2 - S.BladeFlurry:ChargesFractionalP()) * 25)) then
      if HR.Cast(S.BladeFlurry) then return "blade_flurry 57"; end
    end
  ...
  Finish = function()
    ...
    if S.SliceandDice:IsCastableP() and (Player:BuffRemainsP(S.SliceandDiceBuff) < TargetUnit:TimeToDie() and Player:BuffRemainsP(S.SliceandDiceBuff) < (1 + Player:ComboPoints()) * 1.8) then
      if HR.Cast(S.SliceandDice) then return "slice_and_dice 105"; end
    end
    ...
    if S.RolltheBones:IsCastableP() and ((Player:BuffRemainsP(S.RolltheBonesBuff) <= 3 or bool(VarRtbReroll)) and (TargetUnit:TimeToDie() > 20 or Player:BuffRemainsP(S.RolltheBonesBuff) < TargetUnit:TimeToDie())) then
      if HR.Cast(S.RolltheBones) then return "roll_the_bones 111"; end
    end
    ...
    if S.BetweentheEyes:IsCastableP() and (S.AceUpYourSleeve:AzeriteEnabled() or S.Deadshot:AzeriteEnabled()) then
      if HR.Cast(S.BetweentheEyes) then return "between_the_eyes 119"; end
    end
    -- dispatch
    if S.Dispatch:IsCastableP() then
      if HR.Cast(S.Dispatch) then return "dispatch 125"; end
    end
  end
  Stealth = function()
    -- ambush
    if S.Ambush:IsCastableP() then
      if HR.Cast(S.Ambush) then return "ambush 127"; end
    end
  end
  -- call precombat
  ...
  if Everyone.TargetIsValid() then
    -- stealth
    if S.Stealth:IsCastableP() then
      if HR.Cast(S.Stealth) then return "stealth 130"; end
    end
