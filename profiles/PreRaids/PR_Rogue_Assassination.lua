...
local function APL()
  ...
  Cds = function()
    ...
    if S.MarkedForDeath:IsCastableP() then
      if HR.CastTargetIf(S.MarkedForDeath, 15, "min", function(TargetUnit) return Target:TimeToDie() end, function(TargetUnit) return (Cache.EnemiesCount[15] > 1) and (TargetUnit:TimeToDie() < Player:ComboPointsDeficit() * 1.5 or Player:ComboPointsDeficit() >= cp_max_spend) end) then return "marked_for_death 52" end
    end
    ...
    if S.Vendetta:IsCastableP() and (not bool(stealthed.rogue) and TargetUnit:DebuffP(S.RuptureDebuff) and (not S.Subterfuge:IsAvailable() or not S.ShroudedSuffocation:AzeriteEnabled() or TargetUnit:PMultiplier(S.Garrote) > 1)) then
    ...
    if S.Vanish:IsCastableP() and (S.Subterfuge:IsAvailable() and not TargetUnit:DebuffP(S.GarroteDebuff) and bool(VarSingleTarget)) then
    ...
    if S.Vanish:IsCastableP() and (S.Nightstalker:IsAvailable() and not S.Exsanguinate:IsAvailable() and Player:ComboPoints() >= cp_max_spend and TargetUnit:DebuffP(S.VendettaDebuff)) then
    ...
    if S.Vanish:IsCastableP() and (S.Subterfuge:IsAvailable() and (not S.Exsanguinate:IsAvailable() or not bool(VarSingleTarget)) and not bool(stealthed.rogue) and S.Garrote:CooldownUpP() and TargetUnit:DebuffRefreshableCP(S.GarroteDebuff) and (Cache.EnemiesCount[10] <= 3 and Player:ComboPointsDeficit() >= 1 + Cache.EnemiesCount[10] or Cache.EnemiesCount[10] >= 4 and Player:ComboPointsDeficit() >= 4)) then
    ...
    if S.Vanish:IsCastableP() and (S.MasterAssassin:IsAvailable() and not bool(stealthed.all) and master_assassin_remains <= 0 and not TargetUnit:DebuffRefreshableCP(S.RuptureDebuff)) then
    ...
    if S.Exsanguinate:IsCastableP() and (TargetUnit:DebuffRemainsP(S.RuptureDebuff) > 4 + 4 * cp_max_spend and not TargetUnit:DebuffRefreshableCP(S.GarroteDebuff)) then
      if HR.Cast(S.Exsanguinate) then return "exsanguinate 121"; end
    end
    -- toxic_blade,if=dot.rupture.ticking
    if S.ToxicBlade:IsCastableP() and (TargetUnit:DebuffP(S.RuptureDebuff)) then
  ...
  Dot = function()
    ...
    if S.Rupture:IsCastableP() and (S.Exsanguinate:IsAvailable() and ((Player:ComboPoints() >= cp_max_spend and S.Exsanguinate:CooldownRemainsP() < 1) or (not TargetUnit:DebuffP(S.RuptureDebuff) and (HL.CombatTime() > 10 or Player:ComboPoints() >= 2)))) then
  ...
  Stealthed = function()
    ...
    if S.Rupture:IsCastableP() and (Player:ComboPoints() >= 4 and (S.Nightstalker:IsAvailable() or S.Subterfuge:IsAvailable() and S.Exsanguinate:IsAvailable() and S.Exsanguinate:CooldownRemainsP() <= 2 and bool(VarSingleTarget) or not TargetUnit:DebuffP(S.RuptureDebuff)) and TargetUnit:TimeToDie() - TargetUnit:DebuffRemainsP(S.RuptureDebuff) > 6) then
    ...
    if S.Rupture:IsCastableP() and (S.Subterfuge:IsAvailable() and S.ShroudedSuffocation:AzeriteEnabled() and not TargetUnit:DebuffP(S.RuptureDebuff)) then
    ...
    if S.Garrote:IsCastableP() and (S.Subterfuge:IsAvailable() and S.Exsanguinate:IsAvailable() and S.Exsanguinate:CooldownRemainsP() < 1 and Player:PrevGCDP(1, S.Rupture) and TargetUnit:DebuffRemainsP(S.RuptureDebuff) > 5 + 4 * cp_max_spend) then
