...
local function APL()
  ...
  Precombat = function()
    ...
    if S.Recklessness:IsCastableP() and (not S.FuriousSlash:IsAvailable() and not S.RecklessAbandon:IsAvailable()) then
      if HR.Cast(S.Recklessness) then return "recklessness 6"; end
    end
  end
  Movement = function()
    -- heroic_leap
    if S.HeroicLeap:IsCastableP() then
      if HR.Cast(S.HeroicLeap) then return "heroic_leap 12"; end
    end
  end
  SingleTarget = function()
    -- siegebreaker
    if S.Siegebreaker:IsCastableP() then
      if HR.Cast(S.Siegebreaker) then return "siegebreaker 14"; end
    end
    ...
    if S.Bladestorm:IsCastableP() and (Player:PrevGCDP(1, S.Rampage) and (Target:DebuffP(S.SiegebreakerDebuff) or not S.Siegebreaker:IsAvailable())) then
      if HR.Cast(S.Bladestorm) then return "bladestorm 46"; end
    end
    -- dragon_roar,if=buff.enrage.up
    if S.DragonRoar:IsCastableP() and (Player:BuffP(S.EnrageBuff)) then
      if HR.Cast(S.DragonRoar) then return "dragon_roar 54"; end
    end
  ...
  if Everyone.TargetIsValid() then
    ...
    -- potion
    if I.OldWar:IsReady() and Settings.Commons.UsePotions then
      if HR.CastSuggested(I.OldWar) then return "old_war 80"; end
    end
