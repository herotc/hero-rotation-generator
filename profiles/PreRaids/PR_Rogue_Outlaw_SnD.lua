...
local function APL()
  ...
  Cds = function()
    ...
    if S.MarkedForDeath:IsCastableP() then
      if HR.CastTargetIf(S.MarkedForDeath, 35, "min", EvaluateTargetIfFilterMarkedForDeath49, EvaluateTargetIfMarkedForDeath54) then return "marked_for_death 56" end
    end
    ...
    if S.BladeFlurry:IsCastableP() and (Cache.EnemiesCount[8] >= 2 and not Player:BuffP(S.BladeFlurryBuff) and (not (Cache.EnemiesCount[8] > 1) or 0 > 8 or 10000000000 > (2 - S.BladeFlurry:ChargesFractionalP()) * 25)) then
      if HR.Cast(S.BladeFlurry) then return "blade_flurry 59"; end
    end
    ...
    if S.GhostlyStrike:IsCastableP() and (bool(VarBladeFlurrySync) and Player:ComboPointsDeficit() >= 1 + num(Player:BuffP(S.BroadsideBuff))) then
      if HR.Cast(S.GhostlyStrike) then return "ghostly_strike 75"; end
    end
    ...
    if S.KillingSpree:IsCastableP() and (bool(VarBladeFlurrySync) and (Player:EnergyTimeToMaxPredicted() > 5 or Player:EnergyPredicted() < 15)) then
      if HR.Cast(S.KillingSpree) then return "killing_spree 81"; end
    end
    ...
    if S.BladeRush:IsCastableP() and (bool(VarBladeFlurrySync) and Player:EnergyTimeToMaxPredicted() > 1) then
      if HR.Cast(S.BladeRush) then return "blade_rush 85"; end
    end
    ...
    if S.Vanish:IsCastableP() and (not Player:IsStealthedP(true, true) and bool(VarAmbushCondition)) then
      if HR.Cast(S.Vanish) then return "vanish 89"; end
    end
    ...
    if S.Shadowmeld:IsCastableP() and HR.CDsON() and (not Player:IsStealthedP(true, true) and bool(VarAmbushCondition)) then
      if HR.Cast(S.Shadowmeld, Settings.Commons.OffGCDasOffGCD.Racials) then return "shadowmeld 93"; end
    end
  end
  Finish = function()
    ...
    if S.BetweentheEyes:IsCastableP() and (Player:BuffP(S.RuthlessPrecisionBuff) or (S.Deadshot:AzeriteEnabled() or S.AceUpYourSleeve:AzeriteEnabled()) and Player:BuffP(S.RolltheBonesBuff)) then
      if HR.Cast(S.BetweentheEyes) then return "between_the_eyes 97"; end
    end
    ...
    if S.SliceandDice:IsCastableP() and (Player:BuffRemainsP(S.SliceandDiceBuff) < Target:TimeToDie() and Player:BuffRemainsP(S.SliceandDiceBuff) < (1 + Player:ComboPoints()) * 1.8) then
      if HR.Cast(S.SliceandDice) then return "slice_and_dice 107"; end
    end
    ...
    if S.RolltheBones:IsCastableP() and (Player:BuffRemainsP(S.RolltheBonesBuff) <= 3 or bool(VarRtbReroll)) then
      if HR.Cast(S.RolltheBones) then return "roll_the_bones 113"; end
    end
