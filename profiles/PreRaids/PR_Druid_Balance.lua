...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.Starsurge:IsCastableP() and ((Cache.EnemiesCount[40] < 3 and (not Player:BuffP(S.StarlordBuff) or Player:BuffRemainsP(S.StarlordBuff) >= 4) or S.Starsurge:ExecuteTime() * (FutureAstralPower() / 40) > TargetUnit:TimeToDie()) and (not Player:BuffP(S.CelestialAlignmentBuff) and not Player:BuffP(S.IncarnationBuff) or VarAzStreak < 2 or not Player:PrevGCDP(1, S.Starsurge)) and (10000000000 > (Player:BuffStackP(S.LunarEmpowermentBuff) * S.LunarStrike:ExecuteTime() + Player:BuffStackP(S.SolarEmpowermentBuff) * S.SolarWrath:ExecuteTime()) or (FutureAstralPower() + Player:BuffStackP(S.LunarEmpowermentBuff) * 12 + Player:BuffStackP(S.SolarEmpowermentBuff) * 8) >= 96)) then
