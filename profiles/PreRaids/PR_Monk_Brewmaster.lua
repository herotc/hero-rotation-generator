...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if I.LustrousGoldenPlumage:IsReady() then
      if HR.CastSuggested(I.LustrousGoldenPlumage) then return "lustrous_golden_plumage 23"; end
    end
    -- potion
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 25"; end
    end
    -- blood_fury
    ...
    if S.Berserking:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return "berserking 29"; end
    end
    -- lights_judgment
    ...
    if S.Fireblood:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.Fireblood, Settings.Commons.OffGCDasOffGCD.Racials) then return "fireblood 33"; end
    end
    -- ancestral_call
    ...
    if S.InvokeNiuzaotheBlackOx:IsCastableP() and HR.CDsON() and (Target:TimeToDie() > 25) then
      if HR.Cast(S.InvokeNiuzaotheBlackOx, Settings.Brewmaster.OffGCDasOffGCD.InvokeNiuzaotheBlackOx) then return "invoke_niuzao_the_black_ox 37"; end
    end
    ...
    if S.IronskinBrew:IsCastableP() and (Player:BuffDownP(S.BlackoutComboBuff) and incoming_damage_1999ms > (health.max * 0.1 + stagger.last_tick_damage_4) and Player:BuffStackP(S.ElusiveBrawlerBuff) < 2 and not Player:BuffP(S.IronskinBrewBuff)) then
      if HR.Cast(S.IronskinBrew, Settings.Brewmaster.OffGCDasOffGCD.IronskinBrew) then return "ironskin_brew 39"; end
    end
    ...
    if S.IronskinBrew:IsCastableP() and (S.Brews:ChargesFractionalP() > 1 and S.BlackOxBrew:CooldownRemainsP() < 3) then
      if HR.Cast(S.IronskinBrew, Settings.Brewmaster.OffGCDasOffGCD.IronskinBrew) then return "ironskin_brew 47"; end
    end
    ...
    if S.PurifyingBrew:IsCastableP() and (stagger.pct > (6 * (3 - (S.Brews:ChargesFractionalP()))) and (stagger.last_tick_damage_1 > ((0.02 + 0.001 * (3 - S.Brews:ChargesFractionalP())) * stagger.last_tick_damage_30))) then
      if HR.Cast(S.PurifyingBrew, Settings.Brewmaster.OffGCDasOffGCD.PurifyingBrew) then return "purifying_brew 53"; end
    end
    ...
    if S.BlackOxBrew:IsCastableP() and (S.Brews:ChargesFractionalP() < 0.5) then
      if HR.Cast(S.BlackOxBrew, Settings.Brewmaster.OffGCDasOffGCD.BlackOxBrew) then return "black_ox_brew 59"; end
    end
    ...
    if S.BlackOxBrew:IsCastableP() and ((Player:EnergyPredicted() + (Player:EnergyRegen() * S.KegSmash:CooldownRemainsP())) < 40 and Player:BuffDownP(S.BlackoutComboBuff) and S.KegSmash:CooldownUpP()) then
      if HR.Cast(S.BlackOxBrew, Settings.Brewmaster.OffGCDasOffGCD.BlackOxBrew) then return "black_ox_brew 63"; end
    end
    -- keg_smash,if=spell_targets>=2
    if S.KegSmash:IsCastableP() and (Cache.EnemiesCount[8] >= 2) then
      if HR.Cast(S.KegSmash) then return "keg_smash 71"; end
    end
    ...
    if S.TigerPalm:IsCastableP() and (S.RushingJadeWind:IsAvailable() and Player:BuffP(S.BlackoutComboBuff) and Player:BuffP(S.RushingJadeWindBuff)) then
      if HR.Cast(S.TigerPalm) then return "tiger_palm 79"; end
    end
    ...
    if S.TigerPalm:IsCastableP() and ((S.InvokeNiuzaotheBlackOx:IsAvailable() or S.SpecialDelivery:IsAvailable()) and Player:BuffP(S.BlackoutComboBuff)) then
      if HR.Cast(S.TigerPalm) then return "tiger_palm 87"; end
    end
    -- blackout_strike
    if S.BlackoutStrike:IsCastableP() then
      if HR.Cast(S.BlackoutStrike) then return "blackout_strike 95"; end
    end
    -- keg_smash
    if S.KegSmash:IsCastableP() then
      if HR.Cast(S.KegSmash) then return "keg_smash 97"; end
    end
    -- rushing_jade_wind,if=buff.rushing_jade_wind.down
    if S.RushingJadeWind:IsCastableP() and (Player:BuffDownP(S.RushingJadeWindBuff)) then
      if HR.Cast(S.RushingJadeWind) then return "rushing_jade_wind 99"; end
    end
    ...
    if S.BreathofFire:IsCastableP() and (Player:BuffDownP(S.BlackoutComboBuff) and (Player:HasNotHeroism() or (Player:HasHeroism() and true and Target:DebuffRefreshableCP(S.BreathofFireDotDebuff)))) then
      if HR.Cast(S.BreathofFire) then return "breath_of_fire 103"; end
    end
    -- chi_burst
    if S.ChiBurst:IsCastableP() then
      if HR.Cast(S.ChiBurst) then return "chi_burst 109"; end
    end
    -- chi_wave
    if S.ChiWave:IsCastableP() then
      if HR.Cast(S.ChiWave) then return "chi_wave 111"; end
    end
    ...
    if S.TigerPalm:IsCastableP() and (not S.BlackoutCombo:IsAvailable() and S.KegSmash:CooldownRemainsP() > Player:GCD() and (Player:EnergyPredicted() + (Player:EnergyRegen() * (S.KegSmash:CooldownRemainsP() + Player:GCD()))) >= 65) then
      if HR.Cast(S.TigerPalm) then return "tiger_palm 113"; end
    end
    -- arcane_torrent,if=energy<31
    if S.ArcaneTorrent:IsCastableP() and HR.CDsON() and (Player:EnergyPredicted() < 31) then
      if HR.Cast(S.ArcaneTorrent, Settings.Commons.OffGCDasOffGCD.Racials) then return "arcane_torrent 121"; end
    end
    -- rushing_jade_wind
    if S.RushingJadeWind:IsCastableP() then
      if HR.Cast(S.RushingJadeWind) then return "rushing_jade_wind 123"; end
    end
  end
end
