...
local function APL()
  ...
  Precombat = function()
    ...
    if S.BearForm:IsCastableP() then
      if HR.Cast(S.BearForm) then return "bear_form 2140"; end
    end
    -- snapshot_stats
    -- potion
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 2143"; end
    end
  end
  Cooldowns = function()
    -- potion
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 2145"; end
    end
    -- blood_fury
    if S.BloodFury:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.BloodFury, Settings.Commons.OffGCDasOffGCD.Racials) then return "blood_fury 2147"; end
    end
    -- berserking
    if S.Berserking:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return "berserking 2149"; end
    end
    -- arcane_torrent
    if S.ArcaneTorrent:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.ArcaneTorrent, Settings.Commons.OffGCDasOffGCD.Racials) then return "arcane_torrent 2151"; end
    end
    -- lights_judgment
    if S.LightsJudgment:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.LightsJudgment) then return "lights_judgment 2153"; end
    end
    -- fireblood
    if S.Fireblood:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.Fireblood, Settings.Commons.OffGCDasOffGCD.Racials) then return "fireblood 2155"; end
    end
    -- ancestral_call
    if S.AncestralCall:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.AncestralCall, Settings.Commons.OffGCDasOffGCD.Racials) then return "ancestral_call 2157"; end
    end
    -- barkskin,if=buff.bear_form.up
    if S.Barkskin:IsCastableP() and (Player:BuffP(S.BearFormBuff)) then
      if HR.Cast(S.Barkskin) then return "barkskin 2159"; end
    end
    -- lunar_beam,if=buff.bear_form.up
    if S.LunarBeam:IsCastableP() and (Player:BuffP(S.BearFormBuff)) then
      if HR.Cast(S.LunarBeam) then return "lunar_beam 2163"; end
    end
    -- bristling_fur,if=buff.bear_form.up
    if S.BristlingFur:IsCastableP() and (Player:BuffP(S.BearFormBuff)) then
      if HR.Cast(S.BristlingFur) then return "bristling_fur 2167"; end
    end
    -- use_items
  end
  -- call precombat
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.Maul:IsCastableP() and (Player:RageDeficit() < 10 and Cache.EnemiesCount[40] < 4) then
      if HR.Cast(S.Maul) then return "maul 2176"; end
    end
    ...
    if S.Ironfur:IsCastableP() and (S.Ironfur:Cost() == 0 or (Player:Rage() > S.Ironfur:Cost() and S.LayeredMane:AzeriteEnabled() and Cache.EnemiesCount[40] > 2)) then
      if HR.Cast(S.Ironfur) then return "ironfur 2184"; end
    end
    ...
    if S.Pulverize:IsCastableP() and (Target:DebuffStackP(S.ThrashBearDebuff) == dot.thrash_bear.max_stacks) then
      if HR.Cast(S.Pulverize) then return "pulverize 2208"; end
    end
    ...
    if S.Moonfire:IsCastableP() and (Target:DebuffRefreshableCP(S.MoonfireDebuff) and Cache.EnemiesCount[40] < 2) then
      if HR.Cast(S.Moonfire) then return "moonfire 2214"; end
    end
    -- incarnation
    if S.Incarnation:IsCastableP() then
      if HR.Cast(S.Incarnation) then return "incarnation 2226"; end
    end
    ...
    if Thrash():IsCastableP() and ((Player:BuffDownP(S.IncarnationBuff) and Cache.EnemiesCount[40] > 1) or (Player:BuffP(S.IncarnationBuff) and Cache.EnemiesCount[40] > 4)) then
      if HR.Cast(Thrash()) then return "thrash 2228"; end
    end
    ...
    if S.Moonfire:IsCastableP() and (Player:BuffP(S.GalacticGuardianBuff) and Cache.EnemiesCount[40] < 2) then
      if HR.Cast(S.Moonfire) then return "moonfire 2260"; end
    end
    -- thrash
    if Thrash():IsCastableP() then
      if HR.Cast(Thrash()) then return "thrash 2272"; end
    end
    -- maul
    if S.Maul:IsCastableP() then
      if HR.Cast(S.Maul) then return "maul 2274"; end
    end
    ...
    if S.Moonfire:IsCastableP() and (S.PoweroftheMoon:AzeriteRank() > 1 and Cache.EnemiesCount[40] == 1) then
      if HR.Cast(S.Moonfire) then return "moonfire 2276"; end
    end
    -- swipe
    if Swipe():IsCastableP() then
      if HR.Cast(Swipe()) then return "swipe 2288"; end
    end
  end
end
