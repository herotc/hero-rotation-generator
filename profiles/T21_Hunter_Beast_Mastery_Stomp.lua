...
local function APL()
  ...
  if S.ArcaneTorrent:IsCastableP() and HR.CDsON() and (Player:FocusDeficit() >= 30) then
    if HR.Cast(S.ArcaneTorrent, Settings.BeastMastery.OffGCDasOffGCD.ArcaneTorrent) then return ""; end
  end
  -- berserking,if=buff.bestial_wrath.remains>7&(!set_bonus.tier20_2pc|buff.bestial_wrath.remains<11)
  if S.Berserking:IsCastableP() and HR.CDsON() and (Player:BuffRemainsP(S.BestialWrathBuff) > 7 and (not HL.Tier20_2Pc or Player:BuffRemainsP(S.BestialWrathBuff) < 11)) then
  ...
  if S.BloodFury:IsCastableP() and HR.CDsON() and (Player:BuffRemainsP(S.BestialWrathBuff) > 7) then
  ...
  if S.CobraShot:IsCastableP() and (HL.Tier20_2Pc and Cache.EnemiesCount[40] == 1 and not I.QaplaEredunWarOrder:IsEquipped() and (Player:BuffP(S.BestialWrathBuff) and Player:BuffRemainsP(S.BestialWrathBuff) < Player:GCD() * 2) and (not S.DireFrenzy:IsAvailable() or Pet:BuffRemainsP(S.DireFrenzyBuff) > Player:GCD() * 1.2)) then
  ...
  if S.TitansThunder:IsCastableP() and (Player:BuffP(S.BestialWrathBuff)) then
    if HR.Cast(S.TitansThunder) then return ""; end
  end
