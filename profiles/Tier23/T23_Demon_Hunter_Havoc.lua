...
local function EvaluateTargetIfFilterNemesis33(TargetUnit)
  return TargetUnit:TimeToDie()
end

local function EvaluateTargetIfNemesis48(TargetUnit)
...
local function APL()
  ...
  Cooldown = function()
    ...
    if S.Nemesis:IsCastableP() then
      if HR.CastTargetIf(S.Nemesis, 40, "min", EvaluateTargetIfFilterNemesis33, EvaluateTargetIfNemesis48) then return "nemesis 50" end
    end
    ...
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions and (Player:BuffRemainsP(S.MetamorphosisBuff) > 25 or Target:TimeToDie() < 60) then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 55"; end
    end
  ...
  Demonic = function()
    ...
    if S.FelRush:IsCastableP() and (movement.distance > 15 or Player:BuffP(S.OutofRangeBuff)) then
      if HR.Cast(S.FelRush) then return "fel_rush 139"; end
    end
  ...
  Normal = function()
    ...
    if S.FelRush:IsCastableP() and ((bool(VarWaitingForMomentum) or S.FelMastery:IsAvailable()) and (S.FelRush:ChargesP() == 2 or (10000000000 > 10 and 10000000000 > 10))) then
      if HR.Cast(S.FelRush) then return "fel_rush 155"; end
    end
    ...
    if S.FelRush:IsCastableP() and (not S.Momentum:IsAvailable() and 10000000000 > S.FelRush:ChargesP() * 10 and S.DemonBlades:IsAvailable()) then
      if HR.Cast(S.FelRush) then return "fel_rush 245"; end
    end
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.Disrupt:IsCastableP() then
      if HR.Cast(S.Disrupt) then return "disrupt 329"; end
    end
    ...
    if S.PickUpFragment:IsCastableP() and (Player:FuryDeficit() >= 35) then
      if HR.Cast(S.PickUpFragment) then return "pick_up_fragment 333"; end
    end
