...
local function Apl()
  ...
  local function Vf()
    ...
    if S.Mindbender:IsCastableP() and (buff.insanity_drain_stacks.value >= (VarCdTime + (VarHasteEval * num(not AC.Tier20_4Pc)) - (3 * num(AC.Tier20_4Pc) * num((4294967296 < 15)) * num(((Cache.EnemiesCount[40] - (0 * num((0 > 0)))) == 1))) + (5 - 3 * num(AC.Tier20_4Pc)) * num(Player:HasHeroism()) + 2 * num(S.FortressoftheMind:IsAvailable()) * num(AC.Tier20_4Pc)) and (not S.SurrenderToMadness:IsAvailable() or (S.SurrenderToMadness:IsAvailable() and Target:TimeToDie() > VarS2Mcheck - buff.insanity_drain_stacks.value))) then
      if AR.Cast(S.Mindbender) then return ""; end
    end
