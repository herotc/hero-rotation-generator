...
local function APL()
  ...
  local function Precombat()
    ...
    if (true) then
      VarCdTime = (12 + (2 - 2 * num(S.Mindbender:IsAvailable()) * num(HL.Tier20_4Pc)) * num(HL.Tier19_2Pc) + (1 - 3 * num(S.Mindbender:IsAvailable()) * num(HL.Tier20_4Pc)) * num(I.MangazasMadness:IsEquipped()) + (6 + 5 * num(S.Mindbender:IsAvailable())) * num(HL.Tier20_4Pc) + 2 * S.LashofInsanity:ArtifactRank())
    end
    ...
    if (S.SurrenderToMadness:IsAvailable()) then
      VarS2MsetupTime = (0.8 * (83 + (20 + 20 * num(S.FortressoftheMind:IsAvailable())) * num(HL.Tier20_4Pc) - (5 * num(S.Sanlayn:IsAvailable())) + ((33 - 13 * num(HL.Tier20_4Pc)) * num(S.ReaperofSouls:IsAvailable())) + num(HL.Tier19_2Pc) * 4 + 8 * num(I.MangazasMadness:IsEquipped()) + (raw_haste_pct * 10 * (1 + 0.7 * num(HL.Tier20_4Pc))) * (2 + (0.8 * num(HL.Tier19_2Pc)) + (1 * num(S.ReaperofSouls:IsAvailable())) + (2 * S.MassHysteria:ArtifactRank()) - (1 * num(S.Sanlayn:IsAvailable())))))
    end
    -- potion
  ...
  local function Check()
    ...
    if (HL.CombatTime() + Target:TimeToDie() > 450 and HL.CombatTime() + Target:TimeToDie() < 600) then
      VarActorsFightTimeMod = num(true) - ((num(true) - (450) + (HL.CombatTime() + Target:TimeToDie())) / 10)
    end
    -- variable,op=set,name=actors_fight_time_mod,value=((450-(time+target.time_to_die))%5),if=time+target.time_to_die<=450
    if (HL.CombatTime() + Target:TimeToDie() <= 450) then
      VarActorsFightTimeMod = ((450 - (HL.CombatTime() + Target:TimeToDie())) / 5)
    end
  ...
  local function Main()
    ...
    if S.VoidEruption:IsCastableP() and ((S.Mindbender:IsAvailable() and S.Mindbender:CooldownRemainsP() < (VarEruptEval + Player:GCD() * 4 / 3)) or not S.Mindbender:IsAvailable() or HL.Tier20_4Pc) then
  ...
  local function S2M()
    ...
    if S.Silence:IsCastableP() and Settings.General.InterruptEnabled and Target:IsInterruptible() and (I.SephuzsSecret:IsEquipped() and (bool(target.is_add) or Target:IsCasting()) and S.BuffSephuzsSecret:CooldownUpP() and not Player:BuffP(S.SephuzsSecretBuff)) then
      if HR.CastAnnotated(S.Silence, false, "Interrupt") then return ""; end
    end
    -- void_bolt,if=buff.insanity_drain_stacks.value<6&set_bonus.tier19_4pc
    if S.VoidBolt:IsCastableP() and (buff.insanity_drain_stacks.value < 6 and HL.Tier19_4Pc) then
    ...
    if S.MindBomb:IsCastableP() and (I.SephuzsSecret:IsEquipped() and bool(target.is_add) and S.BuffSephuzsSecret:CooldownRemainsP() < 1 and not Player:BuffP(S.SephuzsSecretBuff)) then
      if HR.Cast(S.MindBomb) then return ""; end
    end
    -- shadow_crash,if=talent.shadow_crash.enabled
    if S.ShadowCrash:IsCastableP() and (S.ShadowCrash:IsAvailable()) then
      if HR.Cast(S.ShadowCrash) then return ""; end
    ...
    if S.Mindbender:IsCastableP() and (S.ShadowWordDeath:ChargesP() == 0 and Player:BuffStackP(S.VoidformBuff) > (45 + 25 * num(HL.Tier20_4Pc))) then
    ...
    if S.Berserking:IsCastableP() and HR.CDsON() and (Player:BuffStackP(S.VoidformBuff) >= 65) then
      if HR.Cast(S.Berserking, Settings.Shadow.OffGCDasOffGCD.Berserking) then return ""; end
    end
    ...
    if S.PowerInfusion:IsCastableP() and (S.ShadowWordDeath:ChargesP() == 0 and Player:BuffStackP(S.VoidformBuff) > (45 + 25 * num(HL.Tier20_4Pc)) or Target:TimeToDie() <= 30) then
      if HR.Cast(S.PowerInfusion) then return ""; end
    end
    -- void_bolt
    ...
    if S.Dispersion:IsCastableP() and (current_insanity_drain * Player:GCD() > Player:Insanity() and not Player:BuffP(S.PowerInfusionBuff) or (Player:BuffStackP(S.VoidformBuff) > 76 and S.ShadowWordDeath:ChargesP() == 0 and current_insanity_drain * Player:GCD() > Player:Insanity())) then
      if HR.Cast(S.Dispersion) then return ""; end
    end
  ...
  local function Vf()
    ...
    if S.Mindbender:IsCastableP() and (buff.insanity_drain_stacks.value >= (VarCdTime + (VarHasteEval * num(not HL.Tier20_4Pc)) - (3 * num(HL.Tier20_4Pc) * num((10000000000 < 15)) * num(((Cache.EnemiesCount[40] - (0 * num((0 > 0)))) == 1))) + (5 - 3 * num(HL.Tier20_4Pc)) * num(Player:HasHeroism()) + 2 * num(S.FortressoftheMind:IsAvailable()) * num(HL.Tier20_4Pc)) and (not S.SurrenderToMadness:IsAvailable() or (S.SurrenderToMadness:IsAvailable() and Target:TimeToDie() > VarS2Mcheck - buff.insanity_drain_stacks.value))) then
      if HR.Cast(S.Mindbender) then return ""; end
    end
    ...
    if S.PowerInfusion:IsCastableP() and (buff.insanity_drain_stacks.value >= (VarCdTime + 5 * num(Player:HasHeroism()) * (1 + 1 * num(HL.Tier20_4Pc))) and (not S.SurrenderToMadness:IsAvailable() or (S.SurrenderToMadness:IsAvailable() and Target:TimeToDie() > VarS2Mcheck - (buff.insanity_drain_stacks.value) + 61))) then
      if HR.Cast(S.PowerInfusion) then return ""; end
    end
    ...
    if S.Berserking:IsCastableP() and HR.CDsON() and (Player:BuffStackP(S.VoidformBuff) >= 10 and buff.insanity_drain_stacks.value <= 20 and (not S.SurrenderToMadness:IsAvailable() or (S.SurrenderToMadness:IsAvailable() and Target:TimeToDie() > VarS2Mcheck - (buff.insanity_drain_stacks.value) + 60))) then
      if HR.Cast(S.Berserking, Settings.Shadow.OffGCDasOffGCD.Berserking) then return ""; end
    end
