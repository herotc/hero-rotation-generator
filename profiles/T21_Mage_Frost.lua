...
local function APL()
  ...
  local function Aoe()
    ...
    if S.WaterJet:IsCastableP() and (Player:PrevGCDP(1, S.Frostbolt) and Player:BuffStackP(S.FingersofFrostBuff) < 3 and not bool(Player:BuffStackP(S.BrainFreezeBuff))) then
      if HR.Cast(S.WaterJet) then return ""; end
    end
    ...
    if S.FrostBomb:IsCastableP() and (Target:DebuffRemainsP(S.FrostBombDebuff) < S.IceLance:TravelTime() and bool(Player:BuffStackP(S.FingersofFrostBuff))) then
      if HR.Cast(S.FrostBomb) then return ""; end
    end
  ...
  local function Single()
    ...
    if S.Flurry:IsCastableP() and (Player:PrevGCDP(1, S.Ebonbolt) or bool(Player:BuffStackP(S.BrainFreezeBuff)) and (Player:PrevGCDP(1, S.GlacialSpike) or Player:PrevGCDP(1, S.Frostbolt) and (not S.GlacialSpike:IsAvailable() or Player:BuffStackP(S.IciclesBuff) <= 4 or S.FrozenOrb:CooldownRemainsP() <= 10 and HL.Tier20_2Pc))) then
      if HR.Cast(S.Flurry) then return ""; end
    end
    -- frozen_orb,if=set_bonus.tier20_2pc&buff.fingers_of_frost.react<3
    if S.FrozenOrb:IsCastableP() and (HL.Tier20_2Pc and Player:BuffStackP(S.FingersofFrostBuff) < 3) then
      if HR.Cast(S.FrozenOrb) then return ""; end
    end
    ...
    if S.GlacialSpike:IsCastableP() and (S.FrozenOrb:CooldownRemainsP() > 10 or not HL.Tier20_2Pc) then
      if HR.Cast(S.GlacialSpike) then return ""; end
    end
    -- frostbolt
    if S.Frostbolt:IsCastableP() and (true) then
