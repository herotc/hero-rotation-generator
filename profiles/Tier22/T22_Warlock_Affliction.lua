...
local function EvaluateTargetIfAgony173(TargetUnit)
...
local function EvaluateTargetIfFilterAgony179(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.AgonyDebuff)
end

local function EvaluateTargetIfAgony212(TargetUnit)
...
local function EvaluateTargetIfFilterSiphonLife218(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.SiphonLifeDebuff)
end

local function EvaluateTargetIfSiphonLife257(TargetUnit)
...
local function EvaluateCycleCorruption264(TargetUnit)
...
local function EvaluateCycleDrainSoul427(TargetUnit)
  return TargetUnit:TimeToDie() <= Player:GCD()
end

local function EvaluateTargetIfFilterDrainSoul433(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff)
end

local function EvaluateTargetIfDrainSoul446(TargetUnit)
  return S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe) and not bool(TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff))
end

local function EvaluateTargetIfFilterDrainSoul452(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff)
end

local function EvaluateTargetIfDrainSoul463(TargetUnit)
  return S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe)
end

local function EvaluateCycleShadowBolt472(TargetUnit)
...
local function EvaluateTargetIfFilterShadowBolt488(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff)
end

local function EvaluateTargetIfShadowBolt499(TargetUnit)
  return S.ShadowEmbrace:IsAvailable() and bool(VarMaintainSe)
end

local function EvaluateCycleUnstableAffliction586(TargetUnit)
...
local function EvaluateCycleDrainSoul657(TargetUnit)
  return TargetUnit:TimeToDie() <= Player:GCD() and Player:SoulShardsP() < 5
end

local function EvaluateTargetIfFilterAgony687(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.AgonyDebuff)
end

local function EvaluateTargetIfAgony704(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.AgonyDebuff) <= Player:GCD() + S.ShadowBolt:ExecuteTime() and TargetUnit:TimeToDie() > 8
end

local function EvaluateCycleUnstableAffliction711(TargetUnit)
  return not bool(contagion) and TargetUnit:TimeToDie() <= 8
end

local function EvaluateTargetIfFilterDrainSoul717(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff)
end

local function EvaluateTargetIfDrainSoul732(TargetUnit)
...
local function EvaluateTargetIfFilterShadowBolt738(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.ShadowEmbraceDebuff)
end

local function EvaluateTargetIfShadowBolt765(TargetUnit)
...
local function EvaluateTargetIfFilterPhantomSingularity771(TargetUnit)
  return TargetUnit:TimeToDie()
end

local function EvaluateTargetIfPhantomSingularity778(TargetUnit)
...
local function EvaluateTargetIfFilterVileTaint784(TargetUnit)
  return TargetUnit:TimeToDie()
end

local function EvaluateTargetIfVileTaint787(TargetUnit)
  return HL.CombatTime() > 15 and TargetUnit:TimeToDie() >= 10
end

local function EvaluateTargetIfFilterUnstableAffliction793(TargetUnit)
  return min:contagion
end

local function EvaluateTargetIfUnstableAffliction798(TargetUnit)
...
local function APL()
  ...
  Dots = function()
    ...
    if S.Agony:IsCastableP() then
      if HR.CastTargetIf(S.Agony, 40, "min", EvaluateTargetIfFilterAgony140, EvaluateTargetIfAgony173) then return "agony 175" end
    end
    ...
    if S.Agony:IsCastableP() then
      if HR.CastTargetIf(S.Agony, 40, "min", EvaluateTargetIfFilterAgony179, EvaluateTargetIfAgony212) then return "agony 214" end
    end
    ...
    if S.SiphonLife:IsCastableP() then
      if HR.CastTargetIf(S.SiphonLife, 40, "min", EvaluateTargetIfFilterSiphonLife218, EvaluateTargetIfSiphonLife257) then return "siphon_life 259" end
    end
    ...
    if S.Corruption:IsCastableP() then
      if HR.CastCycle(S.Corruption, 40, EvaluateCycleCorruption264) then return "corruption 282" end
    end
  end
  Fillers = function()
    ...
    if S.UnstableAffliction:IsReadyP() and (S.Deathbolt:CooldownRemainsP() <= Player:GCD() * 2 and Cache.EnemiesCount[5] == 1 + raid_event.invulnerable.up and S.SummonDarkglare:CooldownRemainsP() > 20) then
      if HR.Cast(S.UnstableAffliction) then return "unstable_affliction 283"; end
    end
    ...
    if S.Deathbolt:IsCastableP() and (S.SummonDarkglare:CooldownRemainsP() >= 30 + Player:GCD() or S.SummonDarkglare:CooldownRemainsP() > 140) then
      if HR.Cast(S.Deathbolt) then return "deathbolt 341"; end
    end
    ...
    if S.ShadowBolt:IsCastableP() and (Player:IsMoving() and bool(Player:BuffRemainsP(S.NightfallBuff))) then
      if HR.Cast(S.ShadowBolt) then return "shadow_bolt 347"; end
    end
    ...
    if S.Agony:IsCastableP() and (Player:IsMoving() and not (S.SiphonLife:IsAvailable() and (Player:PrevGCDP(1, S.Agony) and Player:PrevGCDP(2, S.Agony) and Player:PrevGCDP(3, S.Agony)) or Player:PrevGCDP(1, S.Agony))) then
      if HR.Cast(S.Agony) then return "agony 351"; end
    end
    ...
    if S.SiphonLife:IsCastableP() and (Player:IsMoving() and not (Player:PrevGCDP(1, S.SiphonLife) and Player:PrevGCDP(2, S.SiphonLife) and Player:PrevGCDP(3, S.SiphonLife))) then
      if HR.Cast(S.SiphonLife) then return "siphon_life 363"; end
    end
    ...
    if S.Corruption:IsCastableP() and (Player:IsMoving() and not Player:PrevGCDP(1, S.Corruption) and not S.AbsoluteCorruption:IsAvailable()) then
      if HR.Cast(S.Corruption) then return "corruption 371"; end
    end
    ...
    if S.DrainSoul:IsCastableP() then
      if HR.CastCycle(S.DrainSoul, 40, EvaluateCycleDrainSoul427) then return "drain_soul 429" end
    end
    ...
    if S.DrainSoul:IsCastableP() then
      if HR.CastTargetIf(S.DrainSoul, 40, "min", EvaluateTargetIfFilterDrainSoul433, EvaluateTargetIfDrainSoul446) then return "drain_soul 448" end
    end
    ...
    if S.DrainSoul:IsCastableP() then
      if HR.CastTargetIf(S.DrainSoul, 40, "min", EvaluateTargetIfFilterDrainSoul452, EvaluateTargetIfDrainSoul463) then return "drain_soul 465" end
    end
    -- drain_soul,interrupt_global=1,chain=1,interrupt=1
    if S.DrainSoul:IsCastableP() then
      if HR.Cast(S.DrainSoul) then return "drain_soul 466"; end
    end
    ...
    if S.ShadowBolt:IsCastableP() then
      if HR.CastCycle(S.ShadowBolt, 40, EvaluateCycleShadowBolt472) then return "shadow_bolt 484" end
    end
    -- shadow_bolt,target_if=min:debuff.shadow_embrace.remains,if=talent.shadow_embrace.enabled&variable.maintain_se
    if S.ShadowBolt:IsCastableP() then
      if HR.CastTargetIf(S.ShadowBolt, 40, "min", EvaluateTargetIfFilterShadowBolt488, EvaluateTargetIfShadowBolt499) then return "shadow_bolt 501" end
    end
    -- shadow_bolt
    if S.ShadowBolt:IsCastableP() then
      if HR.Cast(S.ShadowBolt) then return "shadow_bolt 502"; end
    end
  end
  Spenders = function()
    ...
    if S.UnstableAffliction:IsReadyP() and (S.SummonDarkglare:CooldownRemainsP() <= Player:SoulShardsP() * S.UnstableAffliction:ExecuteTime() and (not S.Deathbolt:IsAvailable() or S.Deathbolt:CooldownRemainsP() <= Player:SoulShardsP() * S.UnstableAffliction:ExecuteTime())) then
      if HR.Cast(S.UnstableAffliction) then return "unstable_affliction 504"; end
    end
    ...
    if S.SeedofCorruption:IsCastableP() and (bool(VarUseSeed)) then
      if HR.Cast(S.SeedofCorruption) then return "seed_of_corruption 536"; end
    end
    ...
    if S.UnstableAffliction:IsReadyP() and (not bool(VarUseSeed) and not Player:PrevGCDP(1, S.SummonDarkglare) and (S.Deathbolt:IsAvailable() and S.Deathbolt:CooldownRemainsP() <= S.UnstableAffliction:ExecuteTime() and not S.CascadingCalamity:AzeriteEnabled() or (Player:SoulShardsP() >= 5 and Cache.EnemiesCount[5] < 2 or Player:SoulShardsP() >= 2 and Cache.EnemiesCount[5] >= 2) and Target:TimeToDie() > 4 + S.UnstableAffliction:ExecuteTime() and Cache.EnemiesCount[5] == 1 or Target:TimeToDie() <= 8 + S.UnstableAffliction:ExecuteTime() * Player:SoulShardsP())) then
      if HR.Cast(S.UnstableAffliction) then return "unstable_affliction 540"; end
    end
    ...
    if S.UnstableAffliction:IsReadyP() and (not bool(VarUseSeed) and contagion <= S.UnstableAffliction:CastTime() + VarPadding) then
      if HR.Cast(S.UnstableAffliction) then return "unstable_affliction 570"; end
    end
    ...
    if S.UnstableAffliction:IsReadyP() then
      if HR.CastCycle(S.UnstableAffliction, 40, EvaluateCycleUnstableAffliction586) then return "unstable_affliction 608" end
    end
  end
  -- call precombat
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.DrainSoul:IsCastableP() then
      if HR.CastCycle(S.DrainSoul, 40, EvaluateCycleDrainSoul657) then return "drain_soul 659" end
    end
    ...
    if S.Haunt:IsCastableP() and (Cache.EnemiesCount[5] <= 2 + raid_event.invulnerable.up) then
      if HR.Cast(S.Haunt) then return "haunt 660"; end
    end
    ...
    if S.SummonDarkglare:IsCastableP() and HR.CDsON() and (Target:DebuffP(S.AgonyDebuff) and Target:DebuffP(S.CorruptionDebuff) and (ActiveUAs() == 5 or Player:SoulShardsP() == 0) and (not S.PhantomSingularity:IsAvailable() or bool(S.PhantomSingularity:CooldownRemainsP())) and (not S.Deathbolt:IsAvailable() or S.Deathbolt:CooldownRemainsP() <= Player:GCD() or not bool(S.Deathbolt:CooldownRemainsP()) or Cache.EnemiesCount[5] > 1 + raid_event.invulnerable.up)) then
      if HR.Cast(S.SummonDarkglare, Settings.Affliction.GCDasOffGCD.SummonDarkglare) then return "summon_darkglare 662"; end
    end
    ...
    if S.Deathbolt:IsCastableP() and (bool(S.SummonDarkglare:CooldownRemainsP()) and Cache.EnemiesCount[5] == 1 + raid_event.invulnerable.up) then
      if HR.Cast(S.Deathbolt) then return "deathbolt 680"; end
    end
    -- agony,target_if=min:dot.agony.remains,if=remains<=gcd+action.shadow_bolt.execute_time&target.time_to_die>8
    if S.Agony:IsCastableP() then
      if HR.CastTargetIf(S.Agony, 40, "min", EvaluateTargetIfFilterAgony687, EvaluateTargetIfAgony704) then return "agony 706" end
    end
    -- unstable_affliction,target_if=!contagion&target.time_to_die<=8
    if S.UnstableAffliction:IsReadyP() then
      if HR.CastCycle(S.UnstableAffliction, 40, EvaluateCycleUnstableAffliction711) then return "unstable_affliction 713" end
    end
    ...
    if S.DrainSoul:IsCastableP() then
      if HR.CastTargetIf(S.DrainSoul, 40, "min", EvaluateTargetIfFilterDrainSoul717, EvaluateTargetIfDrainSoul732) then return "drain_soul 734" end
    end
    ...
    if S.ShadowBolt:IsCastableP() then
      if HR.CastTargetIf(S.ShadowBolt, 40, "min", EvaluateTargetIfFilterShadowBolt738, EvaluateTargetIfShadowBolt765) then return "shadow_bolt 767" end
    end
    ...
    if S.PhantomSingularity:IsCastableP() then
      if HR.CastTargetIf(S.PhantomSingularity, 40, "max", EvaluateTargetIfFilterPhantomSingularity771, EvaluateTargetIfPhantomSingularity778) then return "phantom_singularity 780" end
    end
    -- vile_taint,target_if=max:target.time_to_die,if=time>15&target.time_to_die>=10
    if S.VileTaint:IsCastableP() then
      if HR.CastTargetIf(S.VileTaint, 40, "max", EvaluateTargetIfFilterVileTaint784, EvaluateTargetIfVileTaint787) then return "vile_taint 789" end
    end
    -- unstable_affliction,target_if=min:contagion,if=!variable.use_seed&soul_shard=5
    if S.UnstableAffliction:IsReadyP() then
      if HR.CastTargetIf(S.UnstableAffliction, 40, "min", EvaluateTargetIfFilterUnstableAffliction793, EvaluateTargetIfUnstableAffliction798) then return "unstable_affliction 800" end
    end
    ...
    if S.SeedofCorruption:IsCastableP() and (bool(VarUseSeed) and Player:SoulShardsP() == 5) then
      if HR.Cast(S.SeedofCorruption) then return "seed_of_corruption 801"; end
    end
    ...
    if S.PhantomSingularity:IsCastableP() and (HL.CombatTime() <= 35) then
      if HR.Cast(S.PhantomSingularity, Settings.Affliction.GCDasOffGCD.PhantomSingularity) then return "phantom_singularity 807"; end
    end
    -- vile_taint,if=time<15
    if S.VileTaint:IsCastableP() and (HL.CombatTime() < 15) then
      if HR.Cast(S.VileTaint) then return "vile_taint 809"; end
    end
    ...
    if S.DarkSoul:IsCastableP() and HR.CDsON() and (S.SummonDarkglare:CooldownRemainsP() < 10 and bool(Target:DebuffRemainsP(S.PhantomSingularityDebuff)) or Target:TimeToDie() < 20 + Player:GCD() or Cache.EnemiesCount[5] > 1 + raid_event.invulnerable.up) then
      if HR.Cast(S.DarkSoul, Settings.Affliction.GCDasOffGCD.DarkSoul) then return "dark_soul 811"; end
    end
    -- berserking
    if S.Berserking:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return "berserking 817"; end
    end
