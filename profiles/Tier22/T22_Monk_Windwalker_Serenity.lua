...
local function EvaluateTargetIfFilterRisingSunKick31(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfRisingSunKick44(TargetUnit)
...
local function EvaluateTargetIfFilterTigerPalm64(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfTigerPalm77(TargetUnit)
  return Player:ChiMax() - Player:Chi() >= 2 and (Player:EnergyPredicted() > 56 or Player:BuffDownP(S.RushingJadeWindBuff)) and (not S.HitCombo:IsAvailable() or not Player:PrevGCDP(1, S.TigerPalm))
end

local function EvaluateTargetIfFilterBlackoutKick89(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfBlackoutKick104(TargetUnit)
...
local function EvaluateTargetIfFilterRisingSunKick140(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfRisingSunKick155(TargetUnit)
  return Cache.EnemiesCount[8] < 3 or Player:PrevGCDP(1, S.SpinningCraneKick)
end

local function EvaluateTargetIfFilterBlackoutKick199(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfFilterRisingSunKick232(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfRisingSunKick239(TargetUnit)
  return Player:Chi() >= 5
end

local function EvaluateTargetIfFilterRisingSunKick247(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfFilterBlackoutKick276(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfBlackoutKick293(TargetUnit)
...
local function EvaluateTargetIfFilterTigerPalm309(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfTigerPalm320(TargetUnit)
  return not Player:PrevGCDP(1, S.TigerPalm) and Player:ChiMax() - Player:Chi() >= 2 and (Player:BuffDownP(S.RushingJadeWindBuff) or Player:EnergyPredicted() > 56)
end

local function EvaluateTargetIfFilterTigerPalm370(TargetUnit)
  return TargetUnit:DebuffRemainsP(S.MarkoftheCraneDebuff)
end

local function EvaluateTargetIfTigerPalm383(TargetUnit)
...
local function APL()
  ...
  Aoe = function()
    ...
    if S.RisingSunKick:IsCastableP() then
      if HR.CastTargetIf(S.RisingSunKick, 8, "min", EvaluateTargetIfFilterRisingSunKick31, EvaluateTargetIfRisingSunKick44) then return "rising_sun_kick 46" end
    end
    ...
    if S.TigerPalm:IsCastableP() then
      if HR.CastTargetIf(S.TigerPalm, 8, "min", EvaluateTargetIfFilterTigerPalm64, EvaluateTargetIfTigerPalm77) then return "tiger_palm 79" end
    end
    -- chi_wave
    if S.ChiWave:IsCastableP() then
      if HR.Cast(S.ChiWave) then return "chi_wave 80"; end
    end
    ...
    if S.FlyingSerpentKick:IsCastableP() and (Player:BuffDownP(S.BokProcBuff)) then
      if HR.Cast(S.FlyingSerpentKick) then return "flying_serpent_kick 82"; end
    end
    ...
    if S.BlackoutKick:IsCastableP() then
      if HR.CastTargetIf(S.BlackoutKick, 8, "min", EvaluateTargetIfFilterBlackoutKick89, EvaluateTargetIfBlackoutKick104) then return "blackout_kick 106" end
    end
  end
  Cd = function()
    -- invoke_xuen_the_white_tiger
    if S.InvokeXuentheWhiteTiger:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.InvokeXuentheWhiteTiger, Settings.Windwalker.OffGCDasOffGCD.InvokeXuentheWhiteTiger) then return "invoke_xuen_the_white_tiger 107"; end
    end
    -- use_item,name=lustrous_golden_plumage
    if I.LustrousGoldenPlumage:IsReady() then
      if HR.CastSuggested(I.LustrousGoldenPlumage) then return "lustrous_golden_plumage 109"; end
    end
    -- blood_fury
    if S.BloodFury:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.BloodFury, Settings.Commons.OffGCDasOffGCD.Racials) then return "blood_fury 111"; end
    end
    -- berserking
    if S.Berserking:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.Berserking, Settings.Commons.OffGCDasOffGCD.Racials) then return "berserking 113"; end
    end
    ...
    if S.ArcaneTorrent:IsCastableP() and HR.CDsON() and (Player:ChiMax() - Player:Chi() >= 1 and Player:EnergyTimeToMaxPredicted() >= 0.5) then
      if HR.Cast(S.ArcaneTorrent, Settings.Commons.OffGCDasOffGCD.Racials) then return "arcane_torrent 115"; end
    end
    -- lights_judgment
    if S.LightsJudgment:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.LightsJudgment) then return "lights_judgment 117"; end
    end
    -- fireblood
    ...
    if S.AncestralCall:IsCastableP() and HR.CDsON() then
      if HR.Cast(S.AncestralCall, Settings.Commons.OffGCDasOffGCD.Racials) then return "ancestral_call 121"; end
    end
    -- touch_of_death,if=target.time_to_die>9
    if S.TouchofDeath:IsCastableP() and HR.CDsON() and (Target:TimeToDie() > 9) then
      if HR.Cast(S.TouchofDeath) then return "touch_of_death 123"; end
    end
    ...
    if S.StormEarthandFire:IsCastableP() and HR.CDsON() and (S.StormEarthandFire:ChargesP() == 2 or (S.FistsofFury:CooldownRemainsP() <= 6 and Player:Chi() >= 3 and S.RisingSunKick:CooldownRemainsP() <= 1) or Target:TimeToDie() <= 15) then
      if HR.Cast(S.StormEarthandFire, Settings.Windwalker.OffGCDasOffGCD.StormEarthandFire) then return "storm_earth_and_fire 125"; end
    end
    ...
    if S.Serenity:IsCastableP() and HR.CDsON() and (S.RisingSunKick:CooldownRemainsP() <= 2 or Target:TimeToDie() <= 12) then
      if HR.Cast(S.Serenity, Settings.Windwalker.OffGCDasOffGCD.Serenity) then return "serenity 133"; end
    end
  end
  Serenity = function()
    -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains,if=active_enemies<3|prev_gcd.1.spinning_crane_kick
    if S.RisingSunKick:IsCastableP() then
      if HR.CastTargetIf(S.RisingSunKick, 8, "min", EvaluateTargetIfFilterRisingSunKick140, EvaluateTargetIfRisingSunKick155) then return "rising_sun_kick 157" end
    end
    ...
    if S.FistsofFury:IsCastableP() and ((Player:HasHeroism() and Player:PrevGCDP(1, S.RisingSunKick) and not S.SwiftRoundhouse:AzeriteEnabled()) or Player:BuffRemainsP(S.SerenityBuff) < 1 or (Cache.EnemiesCount[8] > 1 and Cache.EnemiesCount[8] < 5)) then
      if HR.Cast(S.FistsofFury) then return "fists_of_fury 158"; end
    end
    ...
    if S.SpinningCraneKick:IsCastableP() and (not Player:PrevGCDP(1, S.SpinningCraneKick) and (Cache.EnemiesCount[8] >= 3 or (Cache.EnemiesCount[8] == 2 and Player:PrevGCDP(1, S.BlackoutKick)))) then
      if HR.Cast(S.SpinningCraneKick) then return "spinning_crane_kick 178"; end
    end
    -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains
    if S.BlackoutKick:IsCastableP() then
      if HR.CastTargetIf(S.BlackoutKick, 8, "min", EvaluateTargetIfFilterBlackoutKick199) then return "blackout_kick 206" end
    end
  end
  St = function()
    ...
    -- whirling_dragon_punch
    if S.WhirlingDragonPunch:IsCastableP() then
      if HR.Cast(S.WhirlingDragonPunch) then return "whirling_dragon_punch 227"; end
    end
    -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains,if=chi>=5
    if S.RisingSunKick:IsCastableP() then
      if HR.CastTargetIf(S.RisingSunKick, 8, "min", EvaluateTargetIfFilterRisingSunKick232, EvaluateTargetIfRisingSunKick239) then return "rising_sun_kick 241" end
    end
    -- fists_of_fury,if=energy.time_to_max>3
    if S.FistsofFury:IsCastableP() and (Player:EnergyTimeToMaxPredicted() > 3) then
      if HR.Cast(S.FistsofFury) then return "fists_of_fury 242"; end
    end
    -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains
    if S.RisingSunKick:IsCastableP() then
      if HR.CastTargetIf(S.RisingSunKick, 8, "min", EvaluateTargetIfFilterRisingSunKick247) then return "rising_sun_kick 254" end
    end
    ...
    if S.RushingJadeWind:IsCastableP() and (Player:BuffDownP(S.RushingJadeWindBuff) and Player:EnergyTimeToMaxPredicted() > 1 and Cache.EnemiesCount[8] > 1) then
      if HR.Cast(S.RushingJadeWind) then return "rushing_jade_wind 255"; end
    end
    ...
    if S.FistoftheWhiteTiger:IsCastableP() and (Player:Chi() <= 2 and (Player:BuffDownP(S.RushingJadeWindBuff) or Player:EnergyPredicted() > 46)) then
      if HR.Cast(S.FistoftheWhiteTiger) then return "fist_of_the_white_tiger 267"; end
    end
    ...
    if S.EnergizingElixir:IsCastableP() and HR.CDsON() and (Player:Chi() <= 3 and Player:EnergyPredicted() < 50) then
      if HR.Cast(S.EnergizingElixir, Settings.Windwalker.OffGCDasOffGCD.EnergizingElixir) then return "energizing_elixir 271"; end
    end
    ...
    if S.BlackoutKick:IsCastableP() then
      if HR.CastTargetIf(S.BlackoutKick, 8, "min", EvaluateTargetIfFilterBlackoutKick276, EvaluateTargetIfBlackoutKick293) then return "blackout_kick 295" end
    end
    -- chi_wave
    if S.ChiWave:IsCastableP() then
      if HR.Cast(S.ChiWave) then return "chi_wave 296"; end
    end
    ...
    if S.ChiBurst:IsCastableP() and (Player:ChiMax() - Player:Chi() >= 1 and Cache.EnemiesCount[8] == 1 or Player:ChiMax() - Player:Chi() >= 2) then
      if HR.Cast(S.ChiBurst) then return "chi_burst 298"; end
    end
    ...
    if S.TigerPalm:IsCastableP() then
      if HR.CastTargetIf(S.TigerPalm, 8, "min", EvaluateTargetIfFilterTigerPalm309, EvaluateTargetIfTigerPalm320) then return "tiger_palm 322" end
    end
    ...
    if S.FlyingSerpentKick:IsCastableP() and (Player:PrevGCDP(1, S.BlackoutKick) and Player:Chi() > 3 and Player:BuffStackP(S.SwiftRoundhouseBuff) < 2) then
      if HR.Cast(S.FlyingSerpentKick) then return "flying_serpent_kick 323"; end
    end
  end
  -- call precombat
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.SpearHandStrike:IsCastableP() and Target:IsInterruptible() and Settings.General.InterruptEnabled and (Target:IsCasting()) then
      if HR.CastAnnotated(S.SpearHandStrike, false, "Interrupt") then return "spear_hand_strike 331"; end
    end
    ...
    if S.RushingJadeWind:IsCastableP() and (S.Serenity:IsAvailable() and S.Serenity:CooldownRemainsP() < 3 and Player:EnergyTimeToMaxPredicted() > 1 and Player:BuffDownP(S.RushingJadeWindBuff)) then
      if HR.Cast(S.RushingJadeWind) then return "rushing_jade_wind 333"; end
    end
    ...
    if S.TouchofKarma:IsCastableP() and (not S.GoodKarma:IsAvailable()) then
      if HR.Cast(S.TouchofKarma, Settings.Windwalker.OffGCDasOffGCD.TouchofKarma) then return "touch_of_karma 341"; end
    end
    ...
    if S.TouchofKarma:IsCastableP() and (S.GoodKarma:IsAvailable()) then
      if HR.Cast(S.TouchofKarma, Settings.Windwalker.OffGCDasOffGCD.TouchofKarma) then return "touch_of_karma 345"; end
    end
    ...
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions and (Player:BuffP(S.SerenityBuff) or Player:BuffP(S.StormEarthandFireBuff) or (not S.Serenity:IsAvailable() and bool(trinket.proc.agility.react)) or Player:HasHeroism() or Target:TimeToDie() <= 60) then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 349"; end
    end
    ...
    if S.FistoftheWhiteTiger:IsCastableP() and ((Player:EnergyTimeToMaxPredicted() < 1 or (S.Serenity:IsAvailable() and S.Serenity:CooldownRemainsP() < 2)) and Player:ChiMax() - Player:Chi() >= 3) then
      if HR.Cast(S.FistoftheWhiteTiger) then return "fist_of_the_white_tiger 361"; end
    end
    ...
    if S.TigerPalm:IsCastableP() then
      if HR.CastTargetIf(S.TigerPalm, 8, "min", EvaluateTargetIfFilterTigerPalm370, EvaluateTargetIfTigerPalm383) then return "tiger_palm 385" end
    end
