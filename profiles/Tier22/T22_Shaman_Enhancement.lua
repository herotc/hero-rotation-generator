...
local function EvaluateCycleStormstrike114(TargetUnit)
...
--- ======= ACTION LISTS =======
local function APL()
  local Precombat, Asc, Cds, Core, Filler, Maintenance, Opener, PriorityBuffs
  UpdateRanges()
  ...
  Cds = function()
    ...
    if S.BloodFury:IsCastableP() and HR.CDsON() and ((S.Ascendance:IsAvailable() and (Player:BuffP(S.AscendanceBuff) or S.Ascendance:CooldownRemainsP() > 50)) or (not S.Ascendance:IsAvailable() and (feral_spirit.remains > 5 or S.FeralSpirit:CooldownRemainsP() > 50))) then
      if HR.Cast(S.BloodFury, Settings.Commons.OffGCDasOffGCD.Racials) then return "blood_fury 47"; end
    end
    ...
    if S.Fireblood:IsCastableP() and HR.CDsON() and ((S.Ascendance:IsAvailable() and (Player:BuffP(S.AscendanceBuff) or S.Ascendance:CooldownRemainsP() > 50)) or (not S.Ascendance:IsAvailable() and (feral_spirit.remains > 5 or S.FeralSpirit:CooldownRemainsP() > 50))) then
      if HR.Cast(S.Fireblood, Settings.Commons.OffGCDasOffGCD.Racials) then return "fireblood 59"; end
    end
    ...
    if S.AncestralCall:IsCastableP() and HR.CDsON() and ((S.Ascendance:IsAvailable() and (Player:BuffP(S.AscendanceBuff) or S.Ascendance:CooldownRemainsP() > 50)) or (not S.Ascendance:IsAvailable() and (feral_spirit.remains > 5 or S.FeralSpirit:CooldownRemainsP() > 50))) then
      if HR.Cast(S.AncestralCall, Settings.Commons.OffGCDasOffGCD.Racials) then return "ancestral_call 71"; end
    end
    ...
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions and (Player:BuffP(S.AscendanceBuff) or not S.Ascendance:IsAvailable() and feral_spirit.remains > 5 or Target:TimeToDie() <= 60) then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 83"; end
    end
    -- feral_spirit
    if S.FeralSpirit:IsCastableP() then
      if HR.Cast(S.FeralSpirit) then return "feral_spirit 89"; end
    end
    -- ascendance,if=cooldown.strike.remains>0
    if S.Ascendance:IsCastableP() and (S.Strike:CooldownRemainsP() > 0) then
      if HR.Cast(S.Ascendance) then return "ascendance 91"; end
    end
    -- earth_elemental
  end
  Core = function()
    ...
    if S.EarthenSpike:IsCastableP() and (bool(VarFurycheck25)) then
      if HR.Cast(S.EarthenSpike) then return "earthen_spike 96"; end
    end
    -- sundering,if=active_enemies>=3
    if S.Sundering:IsCastableP() and (Cache.EnemiesCount[8] >= 3) then
      if HR.Cast(S.Sundering) then return "sundering 100"; end
    end
    ...
    if S.Stormstrike:IsCastableP() then
      if HR.CastCycle(S.Stormstrike, 8, EvaluateCycleStormstrike114) then return "stormstrike 132" end
    end
    -- lava_lash,if=azerite.primal_primer.rank>=2&debuff.primal_primer.stack=10
    if S.LavaLash:IsCastableP() and (S.PrimalPrimer:AzeriteRank() >= 2 and Target:DebuffStackP(S.PrimalPrimerDebuff) == 10) then
      if HR.Cast(S.LavaLash) then return "lava_lash 133"; end
    end
    ...
    if S.Stormstrike:IsCastableP() and (Player:BuffP(S.StormbringerBuff) or (Player:BuffP(S.GatheringStormsBuff) and bool(VarOcpool70) and bool(VarFurycheck35))) then
      if HR.Cast(S.Stormstrike) then return "stormstrike 139"; end
    end
    ...
    if S.CrashLightning:IsCastableP() and (Cache.EnemiesCount[8] >= 3 and bool(VarFurycheck25)) then
      if HR.Cast(S.CrashLightning) then return "crash_lightning 149"; end
    end
    ...
    if S.LightningBolt:IsCastableP() and (S.Overcharge:IsAvailable() and Cache.EnemiesCount[8] == 1 and bool(VarFurycheck45) and Player:Maelstrom() >= 40) then
      if HR.Cast(S.LightningBolt) then return "lightning_bolt 161"; end
    end
    -- lava_lash,if=azerite.primal_primer.rank>=2&debuff.primal_primer.stack>7
    if S.LavaLash:IsCastableP() and (S.PrimalPrimer:AzeriteRank() >= 2 and Target:DebuffStackP(S.PrimalPrimerDebuff) > 7) then
      if HR.Cast(S.LavaLash) then return "lava_lash 173"; end
    end
    ...
    if S.Stormstrike:IsCastableP() and (bool(VarOcpool70) and bool(VarFurycheck35)) then
      if HR.Cast(S.Stormstrike) then return "stormstrike 179"; end
    end
    -- sundering
    if S.Sundering:IsCastableP() then
      if HR.Cast(S.Sundering) then return "sundering 185"; end
    end
    ...
    if S.CrashLightning:IsCastableP() and (S.ForcefulWinds:IsAvailable() and Cache.EnemiesCount[8] > 1 and bool(VarFurycheck25)) then
      if HR.Cast(S.CrashLightning) then return "crash_lightning 187"; end
    end
    -- flametongue,if=talent.searing_assault.enabled
    if S.Flametongue:IsCastableP() and (S.SearingAssault:IsAvailable()) then
      if HR.Cast(S.Flametongue) then return "flametongue 201"; end
    end
    ...
    if S.LavaLash:IsCastableP() and (S.HotHand:IsAvailable() and bool(Player:BuffStackP(S.HotHandBuff))) then
      if HR.Cast(S.LavaLash) then return "lava_lash 205"; end
    end
    ...
    if S.CrashLightning:IsCastableP() and (Cache.EnemiesCount[8] > 1 and bool(VarFurycheck25)) then
      if HR.Cast(S.CrashLightning) then return "crash_lightning 211"; end
    end
  end
  Filler = function()
    ...
    if S.Rockbiter:IsCastableP() and (Player:Maelstrom() < 70 and not Player:BuffP(S.StrengthofEarthBuff)) then
      if HR.Cast(S.Rockbiter) then return "rockbiter 223"; end
    end
    ...
    if S.CrashLightning:IsCastableP() and (S.CrashingStorm:IsAvailable() and bool(VarOcpool60)) then
      if HR.Cast(S.CrashLightning) then return "crash_lightning 227"; end
    end
    ...
    if S.LavaLash:IsCastableP() and (bool(VarOcpool80) and bool(VarFurycheck45)) then
      if HR.Cast(S.LavaLash) then return "lava_lash 233"; end
    end
    -- rockbiter
    if S.Rockbiter:IsCastableP() then
      if HR.Cast(S.Rockbiter) then return "rockbiter 239"; end
    end
    -- flametongue
    if S.Flametongue:IsCastableP() then
      if HR.Cast(S.Flametongue) then return "flametongue 241"; end
    end
  end
  Maintenance = function()
    -- flametongue,if=!buff.flametongue.up
    if S.Flametongue:IsCastableP() and (not Player:BuffP(S.FlametongueBuff)) then
      if HR.Cast(S.Flametongue) then return "flametongue 243"; end
    end
    ...
    if S.Frostbrand:IsCastableP() and (S.Hailstorm:IsAvailable() and not Player:BuffP(S.FrostbrandBuff) and bool(VarFurycheck25)) then
      if HR.Cast(S.Frostbrand) then return "frostbrand 247"; end
    end
    ...
    if S.Flametongue:IsCastableP() and (Player:BuffRemainsP(S.FlametongueBuff) < 4.8 + Player:GCD()) then
      if HR.Cast(S.Flametongue) then return "flametongue 255"; end
    end
    ...
    if S.Frostbrand:IsCastableP() and (S.Hailstorm:IsAvailable() and Player:BuffRemainsP(S.FrostbrandBuff) < 4.8 + Player:GCD() and bool(VarFurycheck25)) then
      if HR.Cast(S.Frostbrand) then return "frostbrand 259"; end
    end
  end
  Opener = function()
    -- rockbiter,if=maelstrom<15&time<gcd
    if S.Rockbiter:IsCastableP() and (Player:Maelstrom() < 15 and HL.CombatTime() < Player:GCD()) then
      if HR.Cast(S.Rockbiter) then return "rockbiter 267"; end
    end
  end
  PriorityBuffs = function()
    -- crash_lightning,if=!buff.crash_lightning.up&active_enemies>1&variable.furyCheck25
    if S.CrashLightning:IsCastableP() and (not Player:BuffP(S.CrashLightningBuff) and Cache.EnemiesCount[8] > 1 and bool(VarFurycheck25)) then
      if HR.Cast(S.CrashLightning) then return "crash_lightning 269"; end
    end
    -- rockbiter,if=talent.landslide.enabled&!buff.landslide.up&charges_fractional>1.7
    if S.Rockbiter:IsCastableP() and (S.Landslide:IsAvailable() and not Player:BuffP(S.LandslideBuff) and S.Rockbiter:ChargesFractionalP() > 1.7) then
    ...
    if S.FuryofAir:IsCastableP() and (not Player:BuffP(S.FuryofAirBuff) and Player:Maelstrom() >= 20) then
      if HR.Cast(S.FuryofAir) then return "fury_of_air 293"; end
    end
    -- flametongue,if=talent.hot_hand.enabled&!buff.flametongue.up
    if S.Flametongue:IsCastableP() and (S.HotHand:IsAvailable() and not Player:BuffP(S.FlametongueBuff)) then
      if HR.Cast(S.Flametongue) then return "flametongue 301"; end
    end
    ...
    if S.Frostbrand:IsCastableP() and (S.Hailstorm:IsAvailable() and not Player:BuffP(S.FrostbrandBuff) and bool(VarFurycheck25) and (Cache.EnemiesCount[8] < 3 or (S.NaturalHarmony:AzeriteEnabled() and Player:BuffRemainsP(S.NaturalHarmonyFrostBuff) <= 2 * Player:GCD()))) then
    ...
    if S.Flametongue:IsCastableP() and (S.HotHand:IsAvailable() and Player:BuffRemainsP(S.FlametongueBuff) < 4.8 + Player:GCD()) then
      if HR.Cast(S.Flametongue) then return "flametongue 327"; end
    end
    ...
    if S.Frostbrand:IsCastableP() and (S.Hailstorm:IsAvailable() and Player:BuffRemainsP(S.FrostbrandBuff) < 4.8 + Player:GCD() and bool(VarFurycheck25) and (Cache.EnemiesCount[8] < 3 or (S.NaturalHarmony:AzeriteEnabled() and Player:BuffRemainsP(S.NaturalHarmonyFrostBuff) <= 2 * Player:GCD()))) then
      if HR.Cast(S.Frostbrand) then return "frostbrand 333"; end
    end
    ...
    if S.TotemMastery:IsCastableP() and (Player:BuffRemainsP(S.ResonanceTotemBuff) <= 2 * Player:GCD()) then
      if HR.Cast(S.TotemMastery) then return "totem_mastery 353"; end
    end
  end
  -- call precombat
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.WindShear:IsCastableP() and Target:IsInterruptible() and Settings.General.InterruptEnabled then
      if HR.CastAnnotated(S.WindShear, false, "Interrupt") then return "wind_shear 358"; end
    end
    ...
    if (true) then
      local ShouldReturn = PriorityBuffs(); if ShouldReturn then return ShouldReturn; end
    end
