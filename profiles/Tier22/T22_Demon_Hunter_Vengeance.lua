...
local function APL()
  ...
  Precombat = function()
    ...
    if I.ProlongedPower:IsReady() and Settings.Commons.UsePotions then
      if HR.CastSuggested(I.ProlongedPower) then return "prolonged_power 467"; end
    end
  end
  Brand = function()
    -- sigil_of_flame,if=cooldown.fiery_brand.remains<2
    if S.SigilofFlame:IsCastableP() and (S.FieryBrand:CooldownRemainsP() < 2) then
      if HR.Cast(S.SigilofFlame) then return "sigil_of_flame 469"; end
    end
  ...
  Normal = function()
    ...
    if S.SoulCleave:IsCastableP() and (S.SpiritBomb:IsAvailable() and soul_fragments == 0) then
      if HR.Cast(S.SoulCleave) then return "soul_cleave 509"; end
    end
    -- immolation_aura,if=pain<=90
    if S.ImmolationAura:IsCastableP() and (Player:Pain() <= 90) then
      if HR.Cast(S.ImmolationAura) then return "immolation_aura 513"; end
    end
    -- felblade,if=pain<=70
    if S.Felblade:IsCastableP() and (Player:Pain() <= 70) then
      if HR.Cast(S.Felblade) then return "felblade 515"; end
    end
    -- fracture,if=soul_fragments<=3
    if S.Fracture:IsCastableP() and (soul_fragments <= 3) then
      if HR.Cast(S.Fracture) then return "fracture 517"; end
    end
    -- fel_devastation
    if S.FelDevastation:IsCastableP() then
      if HR.Cast(S.FelDevastation) then return "fel_devastation 519"; end
    end
    -- sigil_of_flame
    if S.SigilofFlame:IsCastableP() then
      if HR.Cast(S.SigilofFlame) then return "sigil_of_flame 521"; end
    end
    -- shear
    if S.Shear:IsCastableP() then
      if HR.Cast(S.Shear) then return "shear 523"; end
    end
    -- throw_glaive
    if S.ThrowGlaive:IsCastableP() then
      if HR.Cast(S.ThrowGlaive) then return "throw_glaive 525"; end
    end
  end
  -- call precombat
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.ConsumeMagic:IsCastableP() then
      if HR.Cast(S.ConsumeMagic) then return "consume_magic 529"; end
    end
