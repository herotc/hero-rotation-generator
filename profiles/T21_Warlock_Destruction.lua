...
local function APL()
  ...
  if S.DimensionalRift:IsCastableP() and (S.DimensionalRift:ChargesP() == 3) then
    if HR.Cast(S.DimensionalRift) then return ""; end
  end
  ...
  if S.Immolate:IsCastableP() and (S.RoaringBlaze:IsAvailable() and Target:DebuffRemainsP(S.Immolate) <= S.Immolate:BaseDuration() and not bool(Target:DebuffRemainsP(S.RoaringBlazeDebuff)) and Target:TimeToDie() > 10 and (S.Conflagrate:ChargesP() == 2 + num(HL.Tier19_4Pc) or (S.Conflagrate:ChargesP() >= 1 + num(HL.Tier19_4Pc) and S.Conflagrate:RechargeP() < S.Immolate:CastTime() + Player:GCD()) or Target:TimeToDie() < 24)) then
    if HR.Cast(S.Immolate) then return ""; end
  end
  -- berserking
  ...
  if S.Shadowburn:IsCastableP() and ((S.Shadowburn:ChargesP() == 1 + num(HL.Tier19_4Pc) and S.Shadowburn:RechargeP() < S.ChaosBolt:CastTime() or S.Shadowburn:ChargesP() == 2 + num(HL.Tier19_4Pc)) and FutureShard() < 5) then
    if HR.Cast(S.Shadowburn) then return ""; end
  end
  ...
  if S.Conflagrate:IsCastableP() and (S.RoaringBlaze:IsAvailable() and (S.Conflagrate:ChargesP() == 2 + num(HL.Tier19_4Pc) or (S.Conflagrate:ChargesP() >= 1 + num(HL.Tier19_4Pc) and S.Conflagrate:RechargeP() < Player:GCD()) or Target:TimeToDie() < 24)) then
  ...
  if S.Conflagrate:IsCastableP() and (not S.RoaringBlaze:IsAvailable() and Player:BuffStackP(S.BackdraftBuff) < 3 and (S.Conflagrate:ChargesP() == 1 + num(HL.Tier19_4Pc) and S.Conflagrate:RechargeP() < S.ChaosBolt:CastTime() or S.Conflagrate:ChargesP() == 2 + num(HL.Tier19_4Pc)) and FutureShard() < 5) then
    if HR.Cast(S.Conflagrate) then return ""; end
  end
