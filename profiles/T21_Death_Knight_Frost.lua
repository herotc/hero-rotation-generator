...
Spell.DeathKnight.Frost = {
  ...
  ObliterationBuff                      = Spell(),
  HungeringRuneWeapon                   = Spell(),
  Icecap                                = Spell(207126),
  FrozenPulse                           = Spell(194909),
  HungeringRuneWeaponBuff               = Spell(),
...
local function APL()
  ...
  local function BosPooling()
    ...
    if S.FrostStrike:IsUsableP() and (Player:RunicPowerDeficit() < 5 and HL.Tier19_4Pc and bool(S.BreathofSindragosa:CooldownRemainsP()) and (not S.ShatteringStrikes:IsAvailable() or Target:DebuffStackP(S.RazoriceDebuff) < 5 or S.BreathofSindragosa:CooldownRemainsP() > 6)) then
    ...
    if S.SindragosasFury:IsCastableP() and ((I.ConsortsColdCore:IsEquipped() or Player:BuffP(S.PillarofFrostBuff)) and bool(Player:BuffStackP(S.UnholyStrengthBuff)) and Target:DebuffStackP(S.RazoriceDebuff) == 5) then
      if HR.Cast(S.SindragosasFury) then return ""; end
    end
    ...
    if S.FrostStrike:IsUsableP() and (Player:RunicPowerDeficit() < 30 and (not S.ShatteringStrikes:IsAvailable() or Target:DebuffStackP(S.RazoriceDebuff) < 5 or S.BreathofSindragosa:CooldownRemainsP() > Player:RuneTimeToX(4))) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
    ...
    if S.FrostStrike:IsUsableP() and (S.BreathofSindragosa:CooldownRemainsP() > Player:RuneTimeToX(4) and (not S.ShatteringStrikes:IsAvailable() or Target:DebuffStackP(S.RazoriceDebuff) < 5 or S.BreathofSindragosa:CooldownRemainsP() > 6)) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
  end
  local function BosTicking()
    ...
    if S.FrostStrike:IsUsableP() and (S.ShatteringStrikes:IsAvailable() and Player:RunicPower() < 40 and Player:RuneTimeToX(2) > 2 and bool(S.EmpowerRuneWeapon:CooldownRemainsP()) and Target:DebuffStackP(S.RazoriceDebuff) == 5 and (bool(S.HornofWinter:CooldownRemainsP()) or not S.HornofWinter:IsAvailable())) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
    ...
    if S.HowlingBlast:IsCastableP() and (((Player:RunicPower() >= 20 and HL.Tier19_4Pc) or Player:RunicPower() >= 30) and Player:BuffP(S.RimeBuff)) then
      if HR.Cast(S.HowlingBlast) then return ""; end
    end
    ...
    if S.FrostStrike:IsUsableP() and (HL.Tier20_2Pc and Player:RunicPowerDeficit() <= 15 and Player:Rune() <= 3 and Player:BuffP(S.PillarofFrostBuff) and not S.ShatteringStrikes:IsAvailable()) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
  ...
  local function ColdHeart()
    ...
    if S.ChainsofIce:IsCastableP() and (Player:BuffP(S.PillarofFrostBuff) and Player:BuffRemainsP(S.PillarofFrostBuff) < Player:GCD() and (Player:BuffStackP(S.ColdHeartBuff) >= 11 or (Player:BuffStackP(S.ColdHeartBuff) >= 10 and HL.Tier20_4Pc))) then
      if HR.Cast(S.ChainsofIce) then return ""; end
    end
  ...
  local function Cooldowns()
    ...
    if S.ArcaneTorrent:IsCastableP() and HR.CDsON() and (Player:RunicPowerDeficit() >= 20 and not S.BreathofSindragosa:IsAvailable()) then
    ...
    if S.ArcaneTorrent:IsCastableP() and HR.CDsON() and (Target:DebuffP(S.BreathofSindragosaDebuff) and Player:RunicPowerDeficit() >= 50 and Player:Rune() < 2) then
      if HR.Cast(S.ArcaneTorrent, Settings.Frost.OffGCDasOffGCD.ArcaneTorrent) then return ""; end
    end
    -- blood_fury,if=buff.pillar_of_frost.up
    if S.BloodFury:IsCastableP() and HR.CDsON() and (Player:BuffP(S.PillarofFrostBuff)) then
    ...
    if I.DraughtofSouls:IsReady() and (Player:RuneTimeToX(5) < 3 and (not Target:DebuffP(S.BreathofSindragosaDebuff) or Player:RunicPower() > 60)) then
      if HR.CastSuggested(I.DraughtofSouls) then return ""; end
    end
    ...
    if S.Obliteration:IsCastableP() and (Player:Rune() >= 1 and Player:RunicPower() >= 20 and (not S.FrozenPulse:IsAvailable() or Player:Rune() < 2 or Player:BuffRemainsP(S.PillarofFrostBuff) <= 12) and (not S.GatheringStorm:IsAvailable() or not S.RemorselessWinter:CooldownUpP()) and (Player:BuffP(S.PillarofFrostBuff) or not S.Icecap:IsAvailable())) then
      if HR.Cast(S.Obliteration) then return ""; end
    end
    ...
    if S.HungeringRuneWeapon:IsCastableP() and (not Player:BuffP(S.HungeringRuneWeaponBuff) and Player:RuneTimeToX(2) > Player:GCD() and Player:RunicPower() < 40) then
      if HR.Cast(S.HungeringRuneWeapon) then return ""; end
    end
  end
  local function Obliteration()
    ...
    if S.FrostStrike:IsUsableP() and (not Player:BuffP(S.RimeBuff) or Player:RuneTimeToX(1) >= Player:GCD() or Player:RunicPowerDeficit() < 20) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
  ...
  local function Standard()
    ...
    if S.FrostStrike:IsUsableP() and (S.IcyTalons:IsAvailable() and Player:BuffRemainsP(S.IcyTalonsBuff) <= Player:GCD()) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
    -- frost_strike,if=talent.shattering_strikes.enabled&debuff.razorice.stack=5&buff.gathering_storm.stack<2&!buff.rime.up
    if S.FrostStrike:IsUsableP() and (S.ShatteringStrikes:IsAvailable() and Target:DebuffStackP(S.RazoriceDebuff) == 5 and Player:BuffStackP(S.GatheringStormBuff) < 2 and not Player:BuffP(S.RimeBuff)) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
    ...
    if S.Obliterate:IsCastableP() and ((I.KoltirasNewfoundWill:IsEquipped() and S.FrozenPulse:IsAvailable() and num(HL.Tier19_2Pc) == 1) or Player:RuneTimeToX(4) < Player:GCD() and Player:BuffP(S.HungeringRuneWeaponBuff)) then
      if HR.Cast(S.Obliterate) then return ""; end
    end
    -- frost_strike,if=(!talent.shattering_strikes.enabled|debuff.razorice.stack<5)&runic_power.deficit<10
    if S.FrostStrike:IsUsableP() and ((not S.ShatteringStrikes:IsAvailable() or Target:DebuffStackP(S.RazoriceDebuff) < 5) and Player:RunicPowerDeficit() < 10) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
    -- howling_blast,if=buff.rime.up
    if S.HowlingBlast:IsCastableP() and (Player:BuffP(S.RimeBuff)) then
    ...
    if S.Obliterate:IsCastableP() and ((I.KoltirasNewfoundWill:IsEquipped() and S.FrozenPulse:IsAvailable() and num(HL.Tier19_2Pc) == 1) or Player:RuneTimeToX(5) < Player:GCD()) then
      if HR.Cast(S.Obliterate) then return ""; end
    end
    -- sindragosas_fury,if=(equipped.consorts_cold_core|buff.pillar_of_frost.up)&buff.unholy_strength.react&debuff.razorice.stack=5
    if S.SindragosasFury:IsCastableP() and ((I.ConsortsColdCore:IsEquipped() or Player:BuffP(S.PillarofFrostBuff)) and bool(Player:BuffStackP(S.UnholyStrengthBuff)) and Target:DebuffStackP(S.RazoriceDebuff) == 5) then
    ...
    if S.FrostStrike:IsUsableP() and (Player:RunicPowerDeficit() < 10 and not Player:BuffP(S.HungeringRuneWeaponBuff)) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
    -- frostscythe,if=buff.killing_machine.react&(!equipped.koltiras_newfound_will|spell_targets.frostscythe>=2)
    if S.Frostscythe:IsCastableP() and (bool(Player:BuffStackP(S.KillingMachineBuff)) and (not I.KoltirasNewfoundWill:IsEquipped() or Cache.EnemiesCount[8] >= 2)) then
    ...
    if S.FrostStrike:IsUsableP() and (Player:RunicPowerDeficit() < 20) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
    -- remorseless_winter,if=spell_targets.remorseless_winter>=2
    if S.RemorselessWinter:IsCastableP() and (Cache.EnemiesCount[8] >= 2) then
    ...
    if S.FrostStrike:IsUsableP() and (not (Player:RunicPower() < 50 and S.Obliteration:IsAvailable() and S.Obliteration:CooldownRemainsP() <= Player:GCD())) then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
